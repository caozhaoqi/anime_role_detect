...
- **增强数据收集脚本**：添加基于关键词的采集器，实现定向数据收集
- **实现全面数据增强**：支持风格迁移和随机增强
- **添加自动数据集分割**：80/10/10训练/验证/测试分割
- **启动模型训练**：当前进度2/50轮，准确率从4.31%提升到5.62%
- **优化训练流水线**：MPS加速达到~9.0 batch/s
- **更新前端API集成**：使用实际后端API替代模拟数据
- **修复推理日志问题**：在分类服务中集成全局日志
- **改进FAISS索引管理**：自动重建和加载
- **增强后端API**：同时处理图片和视频文件的检测
- **更新数据统计**：380个总角色，352个有效角色，8644张总图像
