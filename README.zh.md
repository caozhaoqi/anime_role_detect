...
API 网关将在 `http://127.0.0.1:8000` 上运行。所有 API 请求都应发送到此处。
...
后端 API 服务将在 `http://127.0.0.1:8001` 上运行。
...
│   ├── middleware/             # 中间件
│   ├── services/               # 服务层
│   │   ├── api_gateway/        # API 网关服务（端口 8000）
│   │   ├── model_service/      # 模型服务（端口 8888）
│   │   ├── auth_service/       # 认证服务
│   │   ├── cache_service/      # 缓存服务
│   │   └── processor/          # 图片/文本处理
│   ├── config/                # 配置文件
...
sequenceDiagram
    ...
    participant 前端 as 前端 (3001)
    participant 网关 as API网关 (8000)
    participant 后端 as 后端API (8001)
    participant 模型 as 模型服务 (8888)
...
2. 使用凭据登录
3. 从下拉菜单中选择一个模型（默认：resnet18_loli8）
4. 如需多角色检测，请勾选相应选项
5. 上传要识别的角色图片
6. 等待系统分析图片
7. 查看识别结果和置信度
...
| 模型服务 | 8888 | `MODEL_SERVICE_PORT` |
| 前端 | 3001 | `FRONTEND_PORT` |

### 重要说明

- **API 网关**必须在其他服务之前启动，以确保正确的路由
- 网关使用 `trust_env=False` 来绕过系统代理，实现本地通信
- 临时目录（`temp/`）必须存在才能进行图片处理
- 后端 API 和模型服务可以运行在不同的服务器上
