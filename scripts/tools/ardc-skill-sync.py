...
def list_skills():
    ...
    try:
        ...
        response = requests.get(f"{skill_hub_url}/api/skills", headers=headers, timeout=30)
        response.raise_for_status()
    ...
    for skill_path in SKILL_DIR.iterdir():
        ...
        try:
            response = requests.get(
                f"{skill_hub_url}/api/skills/{skill_name}", headers=headers, timeout=10
            )
...
def sync_skills():
    ...
    for skill_path in SKILL_DIR.iterdir():
        ...
        try:
            # 获取远程版本
            response = requests.get(
                f"{skill_hub_url}/api/skills/{skill_name}/download", headers=headers, timeout=30
            )
            response.raise_for_status()
