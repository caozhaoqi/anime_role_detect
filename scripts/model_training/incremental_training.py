...
# ============= 检查点保存与加载 =============
def save_checkpoint(
    model, optimizer, scheduler, epoch, best_acc, train_history, filepath, ema=None
):
    checkpoint = {
        "epoch": epoch,
        "best_acc": best_acc,
...
class WarmupCosineAnnealingLR(torch.optim.lr_scheduler._LRScheduler):
    ...
    def get_lr(self):
        if self.last_epoch < self.warmup_epochs:
            ...
            return [
                self.eta_min + (base_lr - self.eta_min) * cos_decay for base_lr in self.base_lrs
            ]
...
def incremental_train(
    ...
    scheduler = WarmupCosineAnnealingLR(
        optimizer, T_max=NUM_EPOCHS, warmup_epochs=WARMUP_EPOCHS, eta_min=1e-7
    )
    ...
    if resume_checkpoint and os.path.exists(resume_checkpoint):
        start_epoch, best_acc, train_history = load_checkpoint(
            resume_checkpoint, model, optimizer, ema=ema
        )
    ...
    for epoch in range(start_epoch, NUM_EPOCHS):
        ...
        if val_acc > best_acc + EARLY_STOP_DELTA:
            ...
            if patience_counter >= PATIENCE:
                logger.info(
                    f"  ⏹️ 验证准确率连续 {PATIENCE} 轮未提升超过 {EARLY_STOP_DELTA}, 提前停止训练"
                )
                break

        if (epoch + 1) % 5 == 0:
            checkpoint_file = os.path.join(
                CHECKPOINT_DIR, f"incremental_checkpoint_epoch_{epoch+1}.pth"
            )
            save_checkpoint(
                model, optimizer, scheduler, epoch, best_acc, train_history, checkpoint_file, ema
            )
