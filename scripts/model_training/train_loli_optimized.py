...
try:
    ...
    logger = get_logger("train_loli_optimized")
except ModuleNotFoundError:
    import logging
    logging.basicConfig(level=logging.INFO)
    logger = logging.getLogger("train_loli_optimized")
...
def save_model(model, model_type, class_to_idx, accuracy):
    model_dir = os.path.join(MODEL_DIR, f"{model_type}_loli67_20imgs_optimized")
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("开始训练优化版角色分类模型 (67个角色, 每角色20张图片)")
    logger.info("=" * 60)
    ...
    results_file = os.path.join(MODEL_DIR, 'training_results_loli67_20imgs_optimized.json')
