...
def train_model(model, dataloaders, criterion, optimizer, scheduler, num_epochs=NUM_EPOCHS, patience=PATIENCE):
    since = time.time()
...
def main():
    logger.info("="*60)
    logger.info("使用重新整理的数据集训练角色分类模型")
    logger.info("="*60)
    
    data_dir = './data/reorganized_dataset'
    model_dir = './models/efficientnet_b0_loli_reorganized'
    ...
    model, best_acc = train_model(model, dataloaders, criterion, optimizer, scheduler, num_epochs=NUM_EPOCHS)
