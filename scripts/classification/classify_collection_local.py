...
class LocalCollectionClassifier:
    ...
    def _load_model(self):
        ...
        try:
            ...
            # 加载模型
            model_data = torch.load(self.model_path, map_location=torch.device('mps' if torch.backends.mps.is_available() else 'cpu'))
            
            # 提取模型和类别映射
            if 'model_state_dict' in model_data:
                from torchvision.models import efficientnet_b3, EfficientNet_B3_Weights
                self.model = efficientnet_b3(weights=EfficientNet_B3_Weights.DEFAULT)
                self.model.classifier[1] = torch.nn.Linear(self.model.classifier[1].in_features, len(model_data['class_to_idx']))
...
def main():
    ...
    classifier = LocalCollectionClassifier(
