...
# -*- coding: utf-8 -*-
"""
移动补充脚本 - 通过移动方式补充图片到100张
优先级: 1. 从other目录移动 -> 2. URL下载 -> 3. 复制补充
"""
...
def move_from_other(role_pinyin, need_count):
    """从other目录移动图片"""
    ...
    for img in other_imgs[:need_count]:
        ...
        shutil.move(src, dst)  # 使用move而不是copy
        count += 1
...
def main():
    ...
    for pinyin, name in PINYIN_MAPPING.items():
        ...
        # 优先级1: 从other目录移动
        if need_count > 0:
            from_other = move_from_other(pinyin, need_count)
            if from_other > 0:
                print(f"  ✅ 从other目录移动: {from_other} 张")
    ...
    print(f"复制: {total_copied} 张")
    print(f"总计: {total_moved + total_downloaded + total_copied} 张")
    print("=" * 70)
    ...
    all达标 = True
    for pinyin in PINYIN_MAPPING.keys():
        if get_current_count(pinyin) < TARGET_COUNT:
            all达标 = False
            break
