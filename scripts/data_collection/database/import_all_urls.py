...
def ensure_role_exists(db_manager, role_name, roles):
    ...
    # 未匹配到角色，创建新角色记录
    existing = db_manager.execute_query("SELECT id FROM roles WHERE name = ?", (normalized_name,))
...
def import_all_url_files(db_manager, roles):
    ...
    for url_dir in url_dirs:
        ...
        for filename in sorted(os.listdir(url_dir)):
            ...
            role_name = (
                filename.replace("_zip.txt", "")
                .replace("_img.txt", "")
                .replace("_url.txt", "")
                .replace("_result_url.txt", "")
                .replace(".txt", "")
            )
            ...
            try:
                ...
                if imported_count > 0:
                    imported_files += 1
                    print(
                        f"  ✓ {filename}: {imported_count}/{len(urls)} 个URL (角色: {role_display_name})"
                    )
                else:
                    skipped_files += 1
...
def generate_import_report(db_manager):
    ...
    # 按数据源统计
    print("\n按数据源统计:")
    source_stats = db_manager.execute_query("SELECT source, COUNT(*) FROM raw_urls GROUP BY source")
