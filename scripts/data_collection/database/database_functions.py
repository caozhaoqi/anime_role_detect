...
class DatabaseManager:
    ...
    def add_role(
        ...
        try:
            ...
            self.cursor.execute(
                sql, (name, display_name or name, origin, gender, age, hair_color, eye_color)
            )
            self.conn.commit()
            return True
        except Exception as e:
    ...
    def add_annotation(self, image_id, role_id, features, annotation_json):
        ...
        try:
            ...
            self.cursor.execute(
                sql, (image_id, role_id, json.dumps(features), json.dumps(annotation_json))
            )
            self.conn.commit()
            return True
        except Exception as e:
    ...
    def add_image_record(
        self, user_id, image_id, role_id, classification, confidence, tags=None, notes=None
    ):
        """添加分类图片记录"""
        self.connect()
        try:
            self.cursor.execute(
                "INSERT INTO image_records (user_id, image_id, role_id, classification, confidence, tags, notes) VALUES (%s, %s, %s, %s, %s, %s, %s)",
                (user_id, image_id, role_id, classification, confidence, tags, notes),
            )
            self.conn.commit()
            return self.cursor.lastrowid
        except Exception as e:
    ...
    def get_image_records_by_user(self, user_id):
        ...
        self.connect()
        self.cursor.execute(
            "SELECT ir.*, i.file_path, r.name as role_name FROM image_records ir JOIN images i ON ir.image_id = i.id JOIN roles r ON ir.role_id = r.id WHERE ir.user_id = %s ORDER BY ir.created_at DESC",
            (user_id,),
        )
    ...
    def set_config(self, key, value, description=None, category=None):
        ...
        try:
            self.cursor.execute(
                "INSERT INTO configs (key, value, description, category) VALUES (%s, %s, %s, %s) ON DUPLICATE KEY UPDATE value = %s, description = %s, category = %s",
                (key, value, description, category, value, description, category),
            )
            self.conn.commit()
            return True
        except Exception as e:
    ...
    def get_all_configs(self, category=None):
        ...
        if category:
            self.cursor.execute(
                "SELECT * FROM configs WHERE category = %s ORDER BY key", (category,)
            )
        else:
    ...
    def add_model(
        ...
        try:
            self.cursor.execute(
                "INSERT INTO models (name, path, type, architecture, version, accuracy, precision, recall, f1_score) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s) ON DUPLICATE KEY UPDATE path = %s, type = %s, architecture = %s, version = %s, accuracy = %s, precision = %s, recall = %s, f1_score = %s",
                (
                    name,
                    path,
                    type,
    ...
    def add_training_record(
        ...
        try:
            self.cursor.execute(
                "INSERT INTO training_records (model_id, start_time, end_time, duration, epochs, batch_size, learning_rate, train_loss, val_loss, train_accuracy, val_accuracy, best_epoch, best_val_accuracy, notes) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                (
                    model_id,
                    start_time,
                    end_time,
    ...
    def get_training_records_by_model(self, model_id):
        ...
        self.connect()
        self.cursor.execute(
            "SELECT * FROM training_records WHERE model_id = %s ORDER BY start_time DESC",
            (model_id,),
        )
        records = self.cursor.fetchall()
        self.close()
    ...
    def get_all_training_records(self):
        ...
        self.connect()
        self.cursor.execute(
            "SELECT tr.*, m.name as model_name FROM training_records tr JOIN models m ON tr.model_id = m.id ORDER BY tr.start_time DESC"
        )
        records = self.cursor.fetchall()
        self.close()
        return records
    ...
    def add_raw_url(
        self, url, source=None, role_name=None, artwork_id=None, priority=1, metadata=None
    ):
        """添加原始URL地址"""
        self.connect()
        try:
    ...
    def update_url_status(self, url_id, status):
        ...
        try:
            self.cursor.execute(
                "UPDATE raw_urls SET status = %s, updated_at = CURRENT_TIMESTAMP WHERE id = %s",
                (status, url_id),
            )
            self.conn.commit()
    ...
    def add_artwork(
        ...
        try:
            ...
            # 获取插入的ID或已存在的ID
            self.cursor.execute(
                "SELECT id FROM artworks WHERE source = %s AND source_url = %s",
                (source, source_url),
            )
            result = self.cursor.fetchone()
            return result[0] if result else None
    ...
    def get_collection_statistics(self):
        ...
        try:
            ...
            self.cursor.execute(
                'SELECT COUNT(*) FROM download_records WHERE download_status = "success"'
            )
