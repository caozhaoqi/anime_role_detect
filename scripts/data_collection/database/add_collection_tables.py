...
def create_tables():
    ...
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_download_records_url_id ON download_records (url_id)')
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_download_records_role_name ON download_records (role_name)')
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_download_records_download_status ON download_records (download_status)')
    logger.info("创建索引完成")
...
def add_artwork(title=None, artist=None, source=None, source_url=None, original_url=None, 
    ...
    try:
        ...
        # 获取插入的ID或已存在的ID
        cursor.execute('SELECT id FROM artworks WHERE source = ? AND source_url = ?', (source, source_url))
        result = cursor.fetchone()
        return result[0] if result else None
...
def add_download_record(url_id, artwork_id=None, role_name=None, save_path=None, 
...
def update_download_record(record_id, **kwargs):
    ...
    try:
        ...
        sql = f"UPDATE download_records SET {set_clause}, updated_at = CURRENT_TIMESTAMP WHERE id = ?"
...
def update_raw_url_status(url_id, status):
    ...
    try:
        cursor.execute('UPDATE raw_urls SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', (status, url_id))
        conn.commit()
        return cursor.rowcount > 0
