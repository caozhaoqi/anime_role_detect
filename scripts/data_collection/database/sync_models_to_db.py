...
def create_model_tables():
    ...
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_training_records_start_time ON training_records (start_time)"
    )
    logger.info("创建索引完成")
    ...
    for log_file in log_files:
        try:
            ...
            if not model_result:
                ...
                """,
                    (
                        ...
                        model_path,
                        "classification",
                        log_data.get("architecture", "unknown"),
                        "1.0",
                    ),
                )
            ...
            start_time = datetime.fromisoformat(
                log_data.get("start_time", datetime.now().isoformat())
            )
