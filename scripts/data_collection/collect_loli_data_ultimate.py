...
def load_loli_roles():
    ...
    for loli_name in loli_character_names:
        ...
        for role_key, role_info in all_game_roles.items():
            if loli_name.lower() == role_info["chinese_name"].lower() or \
               loli_name.lower() == role_key.lower() or \
               loli_name.lower() in role_info["keywords"]:
...
def main():
    ...
    if urls:
        ...
        with open(stats_file, 'w', encoding='utf-8') as f:
            ...
            stats_to_save["start_time"] = stats_to_save["start_time"].strftime('%Y-%m-%d %H:%M:%S') if stats_to_save["start_time"] else None
            stats_to_save["end_time"] = stats_to_save["end_time"].strftime('%Y-%m-%d %H:%M:%S') if stats_to_save["end_time"] else None
