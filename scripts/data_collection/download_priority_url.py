...
API_BASE = "http://localhost:33333/api/v1.2.5.260305/sis"  # 正确的API地址
TARGET_COUNT = 100
...
def download_images(pinyin, need_count):
    """下载图片"""
    role_dir = os.path.join(DATA_DIR, pinyin)
    ...
    url_file = os.path.join(url_dir, f'{pinyin}.txt')
...
def main():
    ...
    for pinyin, name in PINYIN_MAPPING.items():
        ...
        # 2. 下载图片
        print("  步骤2: 下载图片...")
        downloaded = download_images(pinyin, need_count)
