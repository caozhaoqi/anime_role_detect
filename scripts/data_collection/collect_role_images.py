...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('role_image_collection.log'),
...
def search_images_google(role_name, num=20):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            import re
            # 查找图片链接
            img_links = re.findall(r'"(https?://[^"\s]+\.(?:jpg|jpeg|png|gif|webp))"', response.text)
            # 去重
...
def collect_role_images(role_name):
    ...
    for i, url in enumerate(img_links):
        ...
        if success:
            ...
            logger.info(f"角色 {role_name}: 下载成功 ({success_count}/{len(img_links)}) - {message}")
        else:
            fail_count += 1
            logger.warning(f"角色 {role_name}: 下载失败 ({fail_count}/{len(img_links)}) - {message}")
