...
def organize_dataset():
    ...
    if roles_not_found:
        logger.info(f"  未匹配列表: {', '.join(roles_not_found[:5])}{'...' if len(roles_not_found) > 5 else ''}")
    logger.info("-"*60)
    ...
    for role_dir in sorted(os.listdir(FINAL_DIR)):
        ...
        if os.path.isdir(role_path):
            count = len([f for f in os.listdir(role_path) if os.path.isfile(os.path.join(role_path, f))])
