...
"""
萝莉角色数据采集脚本（终极版）
基于ultimate_data_collector.py，专门用于采集萝莉角色的图片数据
"""
...
MAX_IMAGES_PER_ROLE = 100  # 每个角色最多采集100张图片
...
def load_loli_roles():
    ...
    if not os.path.exists(LOLI_CHARACTERS_FILE):
        print(f"警告: 萝莉角色文件不存在: {LOLI_CHARACTERS_FILE}")
        return loli_roles

    with open(LOLI_CHARACTERS_FILE, "r", encoding="utf-8") as f:
        loli_character_names = [line.strip() for line in f if line.strip()]
    ...
    # 筛选出萝莉角色
    for loli_name in loli_character_names:
        # 检查是否在游戏角色中
        found = False
        for role_key, role_info in all_game_roles.items():
            if (
                loli_name.lower() == role_info["chinese_name"].lower()
                or loli_name.lower() == role_key.lower()
                or loli_name.lower() in role_info["keywords"]
            ):
                loli_roles[role_key] = role_info
                found = True
                break

        # 如果没有找到，创建新的角色配置
        if not found:
            pinyin_name = loli_name.lower().replace(" ", "_").replace("·", "_").replace("'", "")
            loli_roles[pinyin_name] = {
                "chinese_name": loli_name,
                "game": "未知",
                "dir": pinyin_name,
                "keywords": [loli_name, pinyin_name],
                "target_count": MAX_IMAGES_PER_ROLE,
            }
            ...
            for url in img_urls:
                if any(size in url.lower() for size in ["master", "original", "large", "high"]):
                    if not any(
                        ...
                        for exclude in ["icon", "logo", "thumb", "small", "avatar", "profile"]
                    ):
...
def process_downloads(urls, roles):
    ...
    with ThreadPoolExecutor(max_workers=MAX_THREADS) as executor:
        ...
        for i, url in enumerate(urls, start=1):
            ...
            # 检查是否达到目标数量
            current_count = len(
                [f for f in os.listdir(save_dir) if f.endswith((".jpg", ".jpeg", ".png"))]
            )
        ...
        for future, current, total, url, role_key in futures:
            try:
                ...
                if success:
                    print(
                        f"✓ [{processed_count}/{len(futures)}] {roles[role_key]['chinese_name']}: {result}"
                    )
...
def print_statistics(roles):
    ...
    print("\n角色分布 (前10个):")
    sorted_roles = sorted(
        download_stats["role_distribution"].items(), key=lambda x: x[1], reverse=True
    )
...
def main():
    ...
    # 加载萝莉角色
    print("\n加载萝莉角色列表...")
    roles = load_loli_roles()
    print(f"加载了 {len(roles)} 个萝莉角色")
    ...
    for role_key, role_info in roles.items():
        role_dir = os.path.join(DATA_DIR, role_info["dir"])
        os.makedirs(role_dir, exist_ok=True)  # 确保目录存在
        existing_count = len(
            [f for f in os.listdir(role_dir) if f.endswith((".jpg", ".jpeg", ".png"))]
        )
        if existing_count > 0:
    ...
    if urls:
        ...
        stats_file = "./data/loli_collection_stats.json"
        with open(stats_file, "w", encoding="utf-8") as f:
            # 将datetime对象转换为字符串
            stats_to_save = download_stats.copy()
            stats_to_save["start_time"] = (
                stats_to_save["start_time"].strftime("%Y-%m-%d %H:%M:%S")
                if stats_to_save["start_time"]
                else None
            )
            stats_to_save["end_time"] = (
                stats_to_save["end_time"].strftime("%Y-%m-%d %H:%M:%S")
    ...
    duration = (
        (download_stats["end_time"] - download_stats["start_time"]).total_seconds()
        if download_stats["end_time"]
        else 0
    )
