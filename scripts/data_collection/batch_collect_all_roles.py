...
def start_spider(keyword):
    """启动单个关键字爬取"""
    try:
        response = requests.post(f"{SPIDER_API_URL}/spider_start/single", 
                                params={"key_word": keyword}, timeout=TIMEOUT)
        if response.status_code == 200:
...
def get_spider_result(keyword):
    """获取爬取结果"""
    try:
        response = requests.get(f"{SPIDER_API_URL}/spider/result", 
                                params={"keyword": keyword}, timeout=TIMEOUT)
        if response.status_code == 200:
            result = response.json()
            if result.get("code") == 0:
...
def main():
    ...
    # 加载模型类别列表
    model_dir = os.path.join(project_root, 'models', 'efficientnet_b3_loli_optimized_v2_20260529_133654')
