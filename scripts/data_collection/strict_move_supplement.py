...
TRASH_NSFW_DIR = '/Users/caozhaoqi/PycharmProjects/anime_role_detect/data/trash_nsfw'
...
def move_from_directory(source_dir, role_pinyin, need_count):
    """从指定目录移动图片"""
    ...
    source_imgs = [f for f in os.listdir(source_dir) if f.lower().endswith(('.jpg', '.png', '.webp', '.jpeg', '.bmp')) and f not in current_imgs]
    
    count = 0
    for img in source_imgs[:need_count]:
        src = os.path.join(source_dir, img)
        dst = os.path.join(target_path, img)
        shutil.move(src, dst)  # 使用move，不是copy！
        count += 1
...
def main():
    ...
    for pinyin, name in PINYIN_MAPPING.items():
        ...
        # 优先级1: 从organized_images移动
        if need_count > 0:
            other_path = os.path.join(OTHER_DIR, pinyin)
            from_other = move_from_directory(other_path, pinyin, need_count)
            if from_other > 0:
                print(f"  🔄 从organized_images移动: {from_other} 张")
                total_moved += from_other
                supplemented += from_other
                need_count -= from_other
        
        # 优先级2: 从trash_nsfw移动（解除R18限制）
        if need_count > 0:
            nsfw_path = os.path.join(TRASH_NSFW_DIR, pinyin)
            from_nsfw = move_from_directory(nsfw_path, pinyin, need_count)
            if from_nsfw > 0:
                print(f"  🔄 从trash_nsfw移动: {from_nsfw} 张")
                total_moved += from_nsfw
                supplemented += from_nsfw
                need_count -= from_nsfw
        ...
        if current >= TARGET_COUNT:
            ...
            print(f"  ⚠️ 当前: {current} 张 (仍需 {need_count} 张)")
            remaining_roles.append((name, current, need_count))
    
    print("\n" + "=" * 70)
    print(f"移动: {total_moved} 张")
    print(f"下载: {total_downloaded} 张")
    print(f"总计: {total_moved + total_downloaded} 张")
    print("=" * 70)
    ...
    if remaining_roles:
        ...
        for name, current, need in remaining_roles:
            print(f"  {name}: {current}/{TARGET_COUNT} (需补充 {need} 张)")
        print("\n💡 建议: 启动爬虫API服务后重新运行，或手动补充URL文件")
    else:
        print("\n🎉 所有角色均已达标！")
