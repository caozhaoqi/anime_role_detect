...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("incremental_collection.log"), logging.StreamHandler()],
)
...
def run_incremental_collection():
    ...
    for batch_config in config["batch_plan"]:
        ...
        for role_config in batch_config["roles"]:
            ...
            # 计算需要下载的数量
            need_count = min(
                GLOBAL_CONFIG["incremental_count"], role_config["target_count"] - current_count
            )

            if need_count <= 0:
                logger.info(
                    f"角色 {role_name} 已达到目标数量 ({current_count}/{role_config['target_count']})，跳过"
                )
                continue
            ...
            logger.info(
                f"开始增量采集角色 {role_name}，当前 {current_count} 张，目标增加 {need_count} 张"
            )
...
"""
    ...
    logger.info(
        "0 2 * * * /Users/caozhaoqi/PycharmProjects/anime_role_detect/scripts/data_collection/run_incremental.sh"
    )
