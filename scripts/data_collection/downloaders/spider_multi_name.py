...
def get_all_search_names(role_info):
    ...
    # 日文名（过滤无效的）
    if (
        role_info.get("japanese_name")
        and role_info["japanese_name"] != "-"
        and role_info["japanese_name"] != role_info.get("chinese_name")
    ):
...
def spider_multi_name():
    ...
    logger.info(
        f"采集配置: 每个角色最多 {MAX_URLS_PER_ROLE} 个URL，每个名称最多 {MAX_URLS_PER_NAME} 个URL"
    )
