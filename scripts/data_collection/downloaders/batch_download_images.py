...
try:
    ...
    def send_training_progress_notification(*args, **kwargs):
        pass

    def send_training_complete_notification(*args, **kwargs):
        pass

    def send_training_error_notification(*args, **kwargs):
        pass
...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("batch_download_images.log"), logging.StreamHandler()],
)
    ...
    while retries < GLOBAL_CONFIG["max_retries"]:
        try:
            ...
            if GLOBAL_CONFIG["proxy"]:
                proxies = {"http": GLOBAL_CONFIG["proxy"], "https": GLOBAL_CONFIG["proxy"]}
...
def process_role(role_config, batch_config):
    ...
    try:
        ...
        logger.info(
            f"角色 {role_name}: 需要下载 {need_images} 张图片，实际可下载 {len(download_urls)} 张"
        )
        ...
        logger.info(
            f"开始下载角色 {role_name} 的图片，共 {len(download_urls)} 个链接，目标 {target_count} 张"
        )
        ...
        # 获取质量要求
        min_resolution = batch_config.get("quality_requirements", {}).get(
            "min_resolution", GLOBAL_CONFIG["min_resolution"]
        )

        for url in download_urls:
            success, message = download_image(
                url, role_dir, role_name, GLOBAL_CONFIG["timeout"], min_resolution
            )
            if success:
                success_count += 1
                logger.info(
                    f"角色 {role_name}: 下载成功 ({success_count}/{len(download_urls)}) - {message}"
                )
            else:
                fail_count += 1
                logger.warning(
                    f"角色 {role_name}: 下载失败 ({fail_count}/{len(download_urls)}) - {message}"
                )
        ...
        logger.info(
            f"角色 {role_name}: 下载完成，成功 {success_count} 张，失败 {fail_count} 张，总计 {final_count}/{target_count} 张"
        )
...
def process_batch(batch_config):
    ...
    for idx, role_config in enumerate(roles):
        ...
        notification_scheduler.update_stats(
            results, {"total_success": total_success, "total_fail": total_fail}
        )
        check_and_send_scheduled_notification(
            results, {"total_success": total_success, "total_fail": total_fail}
        )
    ...
    try:
        ...
        if args.batch:
            ...
            if not batch_config:
                ...
                send_training_error_notification(
                    stage="数据采集", error_message=f"批次 {args.batch} 不存在"
                )
                return
        ...
        #             break
            ...
            if not found:
                ...
                send_training_error_notification(
                    stage="数据采集", error_message=f"角色 {args.role} 不存在"
                )
        ...
        # 处理所有批次
            ...
            send_notification(
                f"📥 开始处理所有批次\n总批次: {len(config['batch_plan'])}", level="info"
            )
