...
PROJECT_ROOT = Path(
    __file__
).parent.parent.parent.parent  # 脚本在 scripts/data_collection/downloaders/ 目录
        ...
        try:
            if CONFIG_PATH.exists():
                with open(CONFIG_PATH, "r", encoding="utf-8") as f:
                    ...
                    self.receive_id_type = config.get("feishu", {}).get(
...
⏳ 正在下载..."""
    ...
    def send_download_progress(
        self, downloaded: int, total: int, current_role: str, success_rate: float
    ):
        """发送下载进度通知"""
...
class SmartDownloader:
        ...
        if URL_DIR.exists():
            for file in URL_DIR.glob("*_img.txt"):
                ...
                if urls:
                    files.append((role_name, urls))
            logger.info(
                f"从文件加载了 {sum(len(u) for _, u in files)} 个URL，共 {len(files)} 个角色"
            )
    ...
    def download_all(self, progress_interval: int = 100):
        ...
        # 计算成功率
        success_rate = (
            (self.stats["success"] / self.stats["total"] * 100) if self.stats["total"] > 0 else 0
        )
