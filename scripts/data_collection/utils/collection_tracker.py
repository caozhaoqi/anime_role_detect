...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("collection_tracker.log"), logging.StreamHandler()],
)
...
def get_collection_stats():
    ...
    stats = {
        ...
        "completed_batches": len(
            [b for b in status["batches"].values() if b.get("status") == "completed"]
        ),
...
def main():
    ...
    parser.add_argument(
        "command", choices=["status", "report", "update", "complete"], help="执行的命令"
    )
