...
def is_valid_url(url):
    ...
    common_image_domains = [
        'imgur.com', 'i.imgur.com', 'img.reddit.com', 'i.redd.it',
        'pixiv.net', 'i.pximg.net', 'pi.326688.xyz', 'picsum.photos',
        'source.unsplash.com', 'neeko-copilot.bytedance.net'
    ]
...
def clean_urls(url_file):
    ...
    if unique_count > 0:
        ...
        logger.info(f"清理URL文件 {os.path.basename(url_file)}: 原始 {original_count} 条，有效 {valid_count} 条，去重后 {unique_count} 条")
    else:
...
def process_url_files():
    ...
    logger.info(f"URL有效率: {total_valid / total_original * 100:.2f}%" if total_original > 0 else "无URL")
    logger.info("============================================================")
