    ...
    if "accessories" not in enhanced_annotation:
        if "accessories" in role_annotations:
            ...
            if num_accessories > 0:
                enhanced_annotation["accessories"] = random.sample(
                    ENHANCED_ANNOTATIONS["accessories"], num_accessories
                )
            else:
...
def main():
    ...
    logger.info(
        f"扩展率: {total_processed / total_files * 100:.2f}%" if total_files > 0 else "无文件"
    )
    logger.info("============================================================")
