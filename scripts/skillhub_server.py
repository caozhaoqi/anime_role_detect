...
async def register(request: RegisterRequest):
    ...
    try:
        ...
        user = User(
            username=request.username, password=hashed_password, email=request.email, role="user"
        )
        db.add(user)
        db.commit()
...
async def add_review(
    ...
    try:
        ...
        # 检查是否已评论
        existing_review = (
            db.query(Review).filter(Review.skill_id == skill.id, Review.user_id == user.id).first()
        )
        if existing_review:
            raise HTTPException(status_code=400, detail="您已对此技能进行过评论")

        review = Review(skill_id=skill.id, user_id=user.id, rating=rating, comment=comment)
        db.add(review)
...
async def get_reviews(skill_id: str, limit: int = 10):
    ...
    try:
        ...
        reviews = (
            db.query(Review)
            .filter(Review.skill_id == skill.id)
...
async def add_favorite(skill_id: str, user: User = Depends(get_current_user)):
    ...
    try:
        ...
        existing = (
            db.query(Favorite)
            .filter(Favorite.skill_id == skill.id, Favorite.user_id == user.id)
            .first()
        )
        if existing:
...
async def get_rankings(type: str = "downloads", limit: int = 10):
    ...
    try:
        ...
        return {"type": type, "skills": [skill_to_dict(s) for s in skills], "total": len(skills)}
    finally:
        db.close()
...
async def get_all_rankings(limit: int = 5):
    ...
    try:
        ...
        rating_ranking = (
            db.query(Skill)
            .filter(Skill.rating > 0)
            .order_by(Skill.rating.desc())
            .limit(limit)
            .all()
        )
...
async def rollback_version(
    skill_id: str, target_version: str = Query(...), user: User = Depends(get_current_user)
):
    """回滚到指定版本"""
    db = SessionLocal()
    try:
        ...
        version = (
            ...
            .filter(SkillVersion.skill_id == skill.id, SkillVersion.version == target_version)
            .first()
        )
        if not version:
        ...
        history = InstallHistory(
            user_id=user.id, skill_id=skill.id, version=skill.version, action="rollback_from"
        )
        db.add(history)
