            ...
            if file_name.lower().endswith(('.jpg', '.jpeg', '.png', '.webp')):
                ...
                try:
                    ...
                    img = tf.keras.preprocessing.image.load_img(
                        file_path, 
                        target_size=GLOBAL_CONFIG['image_size']
                    )
...
def create_data_generators(images, labels):
    ...
    x_train_val, x_test, y_train_val, y_test = train_test_split(
        images, labels, 
        test_size=GLOBAL_CONFIG['test_split'],
        random_state=42
    )
    ...
    logger.info(f"数据集划分: 训练集 {len(x_train)} 张，验证集 {len(x_val)} 张，测试集 {len(x_test)} 张")
...
def main():
    ...
    # 创建数据生成器
    train_generator, val_generator, test_generator, x_test, y_test = create_data_generators(images, labels)
