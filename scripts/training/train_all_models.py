...
def train(model_name, data_dir=None, model_dir=None):
    ...
    send_feishu_message(f"🚀 {config['name']} 训练任务开始", f"""数据集: {data_dir}
配置:
- Model: {config['name']}
- Epochs: {NUM_EPOCHS}
...
时间: {time.strftime('%Y-%m-%d %H:%M:%S')}""")
    ...
    try:
        ...
        model, best_acc, history = train_model(
            ...
            num_epochs=NUM_EPOCHS, patience=PATIENCE, model_name=model_name
        )
        ...
        send_feishu_message(f"🎉 {config['name']} 训练完成",
                          f"""训练结果:
- ✅ 最佳验证准确率: {best_acc:.4%}
- 📁 模型保存路径: {model_dir}
...
def main():
    ...
    parser.add_argument('--output', type=str, default=BASE_MODEL_DIR,
                       help=f'模型输出目录 (默认: {BASE_MODEL_DIR})')
