...
def save_checkpoint(
    model, optimizer, scheduler, epoch, best_acc, best_loss, train_history, checkpoint_path
):
    """保存训练检查点"""
    checkpoint = {
        "epoch": epoch,
            ...
            for batch_idx, (inputs, labels) in enumerate(dataloaders[phase]):
                ...
                with torch.set_grad_enabled(phase == "train"):
                    ...
                    if phase == "train":
                        ...
                        if batch_idx % 50 == 0:
                            logger.info(
                                f"  Batch {batch_idx}/{batch_count}, Loss: {loss.item():.4f}"
                            )
...
def main():
    ...
    parser.add_argument(
        "--use_pretrained", action="store_true", default=True, help="使用预训练权重"
    )
    parser.add_argument(
