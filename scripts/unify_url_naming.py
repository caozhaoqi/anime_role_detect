...
def unify_url_naming():
    ...
    for role in roles:
        ...
        if len(files_to_merge) > 1:
            merged_count += 1
            print(f"🔄 {chinese} ({english}): 合并 {len(files_to_merge)} 个文件 -> {new_file_name} ({len(all_urls)} URLs)")
        else:
            renamed_count += 1
            print(f"✅ {chinese} ({english}): {files_to_merge[0].split('/')[-1]} -> {new_file_name} ({len(all_urls)} URLs)")
