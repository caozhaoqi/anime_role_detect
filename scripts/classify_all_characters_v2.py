...
def classify_all_characters():
    ...
    # 将结果输出为txt文件
    output_dir = "/Users/caozhaoqi/PycharmProjects/anime_role_detect/auto_spider_img/classified_all_v2"
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    
    # 输出总结果
    with open(os.path.join(output_dir, "all_classification_results.txt"), "w", encoding="utf-8") as f:
        f.write("分类结果统计:\n")
        for key, value in results.items():
            f.write(f"{key}: {value}\n")
        f.write("\n详细分类结果:\n")
