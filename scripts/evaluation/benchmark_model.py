...
def benchmark_model(model, dataloader, device, class_names):
    ...
    for item in report_df[:10]:
        print(
            f"  {item['class']:<20} {item['precision']:>9.1f}% {item['recall']:>9.1f}% {item['f1']:>9.1f}% {item['support']:>10}"
        )
    ...
    sns.heatmap(
        cm_normalized, annot=False, cmap="Blues", ax=axes[1], cbar_kws={"label": "百分比 (%)"}
    )
    ...
    results = {
        ...
        "top3_accuracy": (
            float(top_k_accuracy_score(all_labels, all_probs, k=3) * 100)
            if len(class_names) >= 3
            else None
        ),
        "top5_accuracy": (
            float(top_k_accuracy_score(all_labels, all_probs, k=5) * 100)
