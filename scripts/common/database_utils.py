...
class ImageDatabase:
    ...
    def is_url_downloaded(self, url: str) -> bool:
        ...
        cursor = conn.cursor()
        cursor.execute(
            'SELECT id FROM downloaded_images WHERE url = ? AND status = "success"', (url,)
        )
    ...
    def save_download_record(
        self, url: str, file_path: str, role_name: str, status: str = "success"
    ) -> bool:
        """
        保存下载记录到数据库
    ...
    def get_downloaded_urls(self, role_name: Optional[str] = None) -> List[str]:
        ...
        if role_name:
            cursor.execute(
                'SELECT url FROM downloaded_images WHERE role_name = ? AND status = "success"',
                (role_name,),
            )
        else:
