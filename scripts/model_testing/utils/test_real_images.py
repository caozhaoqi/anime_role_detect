...
def test_real_images():
    ...
    avg_response_time = (
        sum(r["response_time"] for r in results) / total_count if total_count > 0 else 0
    )
    ...
    for i, result in enumerate(results):
        ...
        log(
            f"{i+1}. {status} {result['expected']} -> {result['predicted']} (相似度: {result['similarity']:.4f}, 时间: {result['response_time']:.2f}s)"
        )
