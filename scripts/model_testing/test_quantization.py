...
def test_quantization():
    ...
    cosine_sim = np.dot(features_non_quantized, features_quantized) / (np.linalg.norm(features_non_quantized) * np.linalg.norm(features_quantized))
    ...
    logger.info(f"相似度下降（Core ML vs 量化）: {(similarity_cml - similarity_q) / similarity_cml * 100:.1f}%")
