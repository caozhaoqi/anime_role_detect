...
def main():
    ...
    if args.eval_dir:
        ...
        # 输出每类准确率（只显示部分）
        print("\n📋 部分类别准确率:")
        sorted_classes = sorted(eval_results['per_class'].items(), key=lambda x: x[1]['accuracy'], reverse=True)
        for class_name, stats in sorted_classes[:10]:
            print(f"  {class_name}: {stats['accuracy'] * 100:.2f}% ({stats['correct']}/{stats['total']})")
