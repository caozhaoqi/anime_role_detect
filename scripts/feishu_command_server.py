...
def verify_feishu_sign(encrypt_key: str, msg_signature: str, timestamp: str, nonce: str, content: str) -> bool:
    """验证飞书签名"""
...
def cmd_list(args: list, receive_id: str) -> str:
    ...
    try:
        ...
        return f"📋 **角色列表** (共 {len(roles)} 个)\n\n" + ", ".join(roles[:20]) + ("..." if len(roles) > 20 else "")
    except Exception as e:
    ...
    if event_type == 'im.message.receive_v1':
        ...
        if text:
            ...
            if handler:
                ...
                print("[未知命令]")
                send_feishu_reply(chat_id, "text",
                    f"❓ 未知命令: `{cmd}`\n发送 `help` 查看可用命令")
