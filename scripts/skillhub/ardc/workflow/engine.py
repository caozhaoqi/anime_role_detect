...
class ExecutionContext(BaseModel):
    ...
    node_outputs: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict, description="节点输出缓存"
    )
    started_at: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc), description="开始时间"
    )
...
class WorkflowEngine:
    ...
    async def execute_workflow(
        ...
        context = ExecutionContext(
            workflow_id=workflow_id, execution_id=execution_id, variables=inputs or {}
        )
        ...
        try:
            ...
            for node_id in execution_order:
                ...
                if not result.success:
                    ...
                    return ExecutionResult(
                        success=False, message=f"节点 {node.name} 执行失败", error=result.error
                    )
            ...
            return ExecutionResult(
                success=True, message="工作流执行成功", outputs=context.variables
            )

        except Exception as e:
            context.status = "failed"
            return ExecutionResult(success=False, message=f"工作流执行异常", error=str(e))
