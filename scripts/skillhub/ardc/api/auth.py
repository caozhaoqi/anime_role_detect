...
async def logout(
    ...
    # 将 token 加入黑名单
    blacklisted_token = TokenBlacklist(id=str(uuid.uuid4()), jti=jti, expires_at=expires_at)
...
async def promote_user(
    ...
    return {
        ...
        "user": {"username": user.username, "email": user.email, "is_developer": user.is_developer},
    }


@router.get("/users")
async def list_users(
    current_user: User = Depends(get_current_developer), db: Session = Depends(get_db)
):
    """列出所有用户（需要开发者权限）"""
