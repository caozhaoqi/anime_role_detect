...
可用子命令:
  ...
  help        - 显示帮助信息
    ...
    parser.add_argument(
        'command', 
        choices=['download', 'collect', 'clean', 'stats', 'help'],
        help='要执行的命令'
    )
...
def run_stats(args):
    ...
    # 统计数据目录中的图片数量
    data_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'data', 'organized_images')
    ...
    for role_dir in os.listdir(data_dir):
        ...
        if os.path.isdir(role_path):
            images = [f for f in os.listdir(role_path) if f.lower().endswith(('.jpg', '.jpeg', '.png', '.webp'))]
            count = len(images)
            if count > 0:
                total_images += count
                total_roles += 1
