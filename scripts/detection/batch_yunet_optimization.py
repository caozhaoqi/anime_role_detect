...
def batch_process_dataset(input_dir, output_base_dir, thresholds=[0.3, 0.5]):
    ...
    # 统计结果
    stats = {"total": 0, "no_face": 0, "single_face": 0, "multi_face": 0, "cropped": 0}
    ...
    for i, image_file in enumerate(image_files):
        if i % 100 == 0:
            ...
            print(
                f"当前统计: 无角色={stats['no_face']}, 单人={stats['single_face']}, 多人={stats['multi_face']}, 切割={stats['cropped']}"
            )
            print("=" * 60)
        ...
        try:
            ...
            if len(faces) == 0:
                ...
                # 自动切割多角色图片
                cropped_files = auto_crop_faces(
                    str(image_file), str(multi_face_cropped_dir), padding=0.3
                )
    ...
    print(
        f"数据利用率提升: {(stats['single_face'] + stats['cropped']) / stats['single_face'] * 100:.1f}% (相比仅单人角色)"
    )
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            f"单人角色: {stats['single_face']} ({stats['single_face']/stats['total']*100:.1f}%)\n"
        )
        f.write(
            f"多人角色: {stats['multi_face']} ({stats['multi_face']/stats['total']*100:.1f}%)\n"
        )
        f.write(f"切割出的单人样本: {stats['cropped']}\n")
        f.write(
            f"有效产出率: {(stats['single_face'] + stats['cropped']) / stats['total'] * 100:.1f}%\n"
        )
