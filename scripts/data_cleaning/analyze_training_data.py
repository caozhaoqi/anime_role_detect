...
def analyze_dataset():
    ...
    if small_images:
        ...
        for img_file, quality in small_images[:10]:
            print(f'  {img_file.relative_to(TRAINING_DATASET)}: {quality["width"]}x{quality["height"]}')
    ...
    for role_name in sorted(results.keys()):
        ...
        print(f"{role_name:<20} {data['count']:<6} {data['valid']:<6} {data['invalid']:<6} {data['avg_size']:<10.1f} {issues_str:<20}")
