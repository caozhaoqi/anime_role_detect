    ...
    for role_name in tqdm(os.listdir(data_dir), desc="处理角色"):
        ...
        for img_name in os.listdir(role_dir):
            ...
            if is_bad:
                ...
                low_quality_images.append({"role": role_name, "path": img_path, "reason": reason})
                continue
            ...
            # 保存图片信息
            role_info["images"].append({"filename": img_name, "features": features, "tags": tags})

        # 计算平均值
        if role_info["valid_images"] > 0:
            role_info["feature_summary"]["avg_width"] = round(
                total_width / role_info["valid_images"]
            )
            role_info["feature_summary"]["avg_height"] = round(
                total_height / role_info["valid_images"]
            )
            role_info["feature_summary"]["avg_file_size_kb"] = round(
                total_size / role_info["valid_images"], 2
            )

        # 获取Top标签
        role_info["top_tags"] = sorted(
            role_tags[role_name].items(), key=lambda x: x[1], reverse=True
        )[:10]
...
def find_common_tags(role_tags, min_roles=2, min_count=5):
    ...
    return dict(
        sorted(
            result.items(), key=lambda x: (len(x[1]["roles"]), x[1]["total_count"]), reverse=True
        )
    )
...
def generate_cleaning_report(analysis_result):
    ...
    report = {
        "summary": {
            ...
            "valid_rate": (
                round(analysis_result["valid_images"] / analysis_result["total_images"] * 100, 2)
...
def main():
    ...
    parser.add_argument(
        "--output", type=str, default="./data_cleaning_report.json", help="输出报告路径"
    )
