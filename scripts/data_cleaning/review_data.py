        ...
        try:
            ...
            if file_hash in results["hash_map"]:
                results["duplicates"].append(
                    {"original": results["hash_map"][file_hash], "duplicate": filepath}
                )
            else:
            ...
            if is_bad:
                if "无法读取" in reason:
                    results["damaged"].append({"path": filepath, "reason": reason})
                else:
                    results["low_quality"].append({"path": filepath, "reason": reason})

            results["image_files"] += 1
        except Exception as e:
            results["damaged"].append({"path": filepath, "reason": f"处理异常: {str(e)}"})
...
def print_summary(report):
    ...
    total_issues = (
        ...
        + report["summary"]["low_quality_count"]
        + report["summary"]["damaged_count"]
    )
...
def main():
    ...
    parser.add_argument(
        "--output", type=str, default="data_review_report.json", help="输出报告文件"
    )
