...
class DataStore:
    ...
    def check_duplicate(self, role_id, hash_value):
        """检查同一角色内是否重复"""
        cursor = self.conn.cursor()
        cursor.execute('SELECT id FROM images WHERE role_id = ? AND hash_value = ?', (role_id, hash_value))
    ...
    def insert_image(self, role_id, file_path, role_hashes=None):
        ...
        try:
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (role_id, filename, file_path, img_hash,
                  img_info['width'], img_info['height'], 
                  img_info['file_size'], img_info['quality_score'],
    ...
    def insert_cleaning_record(self, batch_id, stats):
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (batch_id, stats['total'], stats['valid'], stats['invalid'],
              stats.get('duplicate', 0), stats.get('low_resolution', 0),
              stats.get('small_file', 0), stats['start_time'], 
              stats['end_time'], stats['duration']))
        self.conn.commit()
        return cursor.lastrowid
