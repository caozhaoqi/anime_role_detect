...
def find_url_files(role_key):
    """查找角色的URL文件并统计"""
    stats = {"img_url_count": 0, "href_url_count": 0, "total_urls": 0}
...
def generate_markdown_report(role_stats):
    ...
    report_lines.append("")
    report_lines.append(
        "| 序号 | 角色(中文) | 角色(英文) | 所属游戏 | 图片URL | 作品URL | URL总数 | 已下载图片 | 状态 |"
    )
    report_lines.append(
        "|------|------------|------------|----------|---------|---------|---------|------------|------|"
    )
    ...
    for idx, (role_key, stats) in enumerate(sorted_roles, 1):
        ...
        report_lines.append(
            f"| {idx} | {name} | {english} | {game} | {img_count} | {href_count} | {total} | {images} | {status} |"
        )
    ...
    for game, stats in sorted_games:
        report_lines.append(
            f"| {game} | {stats['count']} | {stats['total_urls']} | {stats['total_images']} |"
        )
    ...
    if low_url_roles:
        ...
        for role_key, stats in low_url_roles:
            ...
            needed = 100 - count
            report_lines.append(
                f"| {name} | {english} | {game} | {count} | 需补充 {needed} 个URL |"
            )
