...
def generate_final_report():
    ...
    for role_dir in TARGET_DIR.iterdir():
        ...
        # 获取角色信息
        info = role_info.get(role_dir.name, {
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        ...
        f.write(f"- 平均每个角色: {stats['total_files'] // stats['roles_with_images'] if stats['roles_with_images'] > 0 else 0} 张\n\n")
        ...
        for idx, role in enumerate(role_file_counts, 1):
            f.write(f"| {idx} | {role['chinese_name']} | {role['english_name']} | {role['japanese_name']} | {role['game']} | {role['file_count']} |\n")
        
        f.write("\n## 图片数量分布\n\n")
        f.write("### 500+ 张\n")
        for role in role_file_counts:
            if role['file_count'] >= 500:
                f.write(f"- {role['chinese_name']} ({role['english_name']}): {role['file_count']} 张\n")
