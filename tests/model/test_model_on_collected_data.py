...
class ModelTester:
    ...
    def load_data(self, sample_ratio=1.0):
        ...
        for character_dir in sorted(os.listdir(self.data_dir)):
            ...
            if sample_ratio < 1.0:
                ...
                logger.info(
                    f"角色 '{character_name}': 原始{len(image_files)}张，采样{num_samples}张"
                )

            for img_file in image_files:
                data.append(
                    {"path": str(img_file), "character": character_name, "class_idx": class_idx}
                )
    ...
    def generate_report(self, test_results, output_path="test_report.txt"):
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"准确率: {test_results['accuracy']:.4f} ({test_results['accuracy']*100:.2f}%)\n"
            )
            ...
            sorted_classes = sorted(
                test_results["class_accuracies"].items(),
            ...
            for class_name, metrics in sorted_classes:
                f.write(
                    f"{class_name:<20} {metrics['accuracy']:<10.4f} "
            ...
            for i, (class_name, metrics) in enumerate(sorted_classes[:10], 1):
                f.write(
                    f"{i:2d}. {class_name:<20} 准确率: {metrics['accuracy']:.4f} "
...
def main():
    ...
    parser.add_argument("--data_dir", type=str, default="data/all_characters", help="数据目录")
    parser.add_argument("--batch_size", type=int, default=32, help="批次大小")
    parser.add_argument("--num_workers", type=int, default=4, help="工作进程数")
    parser.add_argument("--output_report", type=str, default="test_report.txt", help="报告输出路径")
    parser.add_argument(
        "--output_json", type=str, default="detailed_results.json", help="详细结果JSON输出路径"
    )
    parser.add_argument(
        "--sample_ratio",
        type=float,
