...
try:
    ...
    try:
        # 检查文件是否存在
        general_classification_path = os.path.join(
            project_root, "src", "core", "classification", "general_classification.py"
        )
        ...
        if os.path.exists(general_classification_path):
            ...
            spec = importlib.util.spec_from_file_location(
                "general_classification", general_classification_path
            )
...
def test_model_accuracy(
    ...
    Returns:
        测试结果字典
    """
    method_names = {"clip": "CLIP", "model": "MobileNetV2", "ensemble": "Ensemble"}
    ...
    for i, (image_path, expected_role) in enumerate(test_data):
        ...
        try:
            ...
            # 根据方法选择分类函数
            if method == "clip":
                predicted_role, similarity, boxes = classifier.classify_image(
                    image_path, use_model=False
                )
            elif method == "model":
                predicted_role, similarity, boxes = classifier.classify_image(
                    image_path, use_model=True
                )
            ...
            logger.info(
                f"  结果: 期望='{expected_role}', 预测='{predicted_role}', 相似度={similarity:.4f}, 正确={is_correct}"
            )
...
def test_model_performance(classifier, test_image_path, iterations=10):
    ...
    return {"clip": clip_stats, "model": model_stats, "ensemble": ensemble_stats}


def generate_ensemble_comparison_report(
    results_list, performance_results, output_dir="ensemble_test_reports"
):
    """生成集成方法对比报告
    ...
    with open(report_md_file, "w", encoding="utf-8") as f:
        ...
        f.write("## 准确率对比\n\n")
        f.write(
            "| 方法 | 测试总数 | 正确预测 | 错误预测 | 准确率 | 平均推理时间 (ms) | 误检数量 |\n"
        )
        f.write(
            "|------|----------|----------|----------|--------|------------------|----------|\n"
        )
        ...
        for results in results_list:
            f.write(
                f"| {results['model_name']} | {results['total_tests']} | {results['correct_predictions']} | {results['incorrect_predictions']} | {results['accuracy']:.2f}% | {results['average_inference_time']:.2f} | {len(results['false_positives'])} |\n"
            )
        ...
        for results in results_list:
            ...
            if results["false_positives"]:
                ...
                for fp in results["false_positives"][:20]:  # 只显示前20个
                    f.write(
                        f"| {os.path.basename(fp['image_path'])} | {fp['expected_role']} | {fp['predicted_role']} | {fp['similarity']:.4f} |\n"
                    )
        ...
        f.write(
            f"- **准确率最高的方法**: {best_accuracy_method['model_name']} ({best_accuracy_method['accuracy']:.2f}%)\n"
        )

        # 找出速度最快的方法
        best_speed_method = min(results_list, key=lambda x: x["average_inference_time"])
        f.write(
            f"- **速度最快的方法**: {best_speed_method['model_name']} ({best_speed_method['average_inference_time']:.2f}ms)\n"
        )
...
def main():
    ...
    parser.add_argument(
        "--max_images_per_class", type=int, default=5, help="每个类别的最大测试图像数量"
    )
    parser.add_argument("--iterations", type=int, default=10, help="性能测试迭代次数")
    parser.add_argument(
        "--output_dir", type=str, default="ensemble_test_reports", help="测试报告输出目录"
    )
    ...
    # 生成集成方法对比报告
    report_file, report_md_file = generate_ensemble_comparison_report(
        test_results, performance_results, args.output_dir
    )
