...
def test_model(test_data, classifier, use_model=False):
    ...
    for i, (image_path, true_label) in enumerate(test_data):
        ...
        try:
            # 分类图像
            predicted_label, similarity, _ = classifier.classify_image(
...
def main():
    ...
    all_results = {
        ...
        "summary": {
            ...
            "best_model": max(
                [clip_results, efficientnet_results, ensemble_results], key=lambda x: x["accuracy"]
            )["model"],
            "best_accuracy": max(
                [clip_results, efficientnet_results, ensemble_results], key=lambda x: x["accuracy"]
            )["accuracy"],
    ...
    print(
        f"最佳模型: {all_results['summary']['best_model']} ({all_results['summary']['best_accuracy']:.4f})"
    )
