...
def test_single_character_processing():
    ...
    image_files = [
        f for f in os.listdir(test_dir) if f.lower().endswith((".jpg", ".jpeg", ".png", ".bmp"))
    ]
    ...
    for img_file in image_files:
        ...
        try:
            ...
            output_path = os.path.join(
                "tests/test_results", f"processed_{os.path.basename(img_file)}"
            )
    ...
    for img_file in image_files:
        ...
        try:
            ...
            for i, character in enumerate(processed_characters):
                output_path = os.path.join(
                    "tests/test_results",
                    f"processed_{os.path.splitext(os.path.basename(img_file))[0]}_char{i+1}.jpg",
                )
