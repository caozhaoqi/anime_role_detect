...
class MidtermOptimizationTest:
    ...
    def test_concurrency_optimization(self):
        ...
        for concurrency in concurrency_levels:
            ...
            logger.info(
                f"并发数 {concurrency}: 总耗时 {total_time:.4f}秒, QPS: {qps:.2f}, 成功率: {success_count / len(test_urls) * 100:.2f}%"
            )

        # 获取动态并发数调整结果
        dynamic_concurrency = concurrency_manager.get_optimal_workers()

        result = {"concurrency_tests": results, "dynamic_concurrency": dynamic_concurrency}
    ...
    def test_monitoring_system(self):
        ...
        result = {"stats": stats, "alerts": alerts, "dashboard_data": dashboard_data}
    ...
    def print_test_summary(self):
        ...
        if "network_optimization" in self.test_results:
            ...
            logger.info(
                f"网络优化: 平均加速比 {net_result.get('speedup', 0):.2f}x, 缓存命中率 {net_result.get('cache_hit_rate', 0):.2f}%"
            )
        ...
        if "concurrency_optimization" in self.test_results:
            ...
            best_result = (
                max(concur_result.get("concurrency_tests", []), key=lambda x: x["qps"])
                if "concurrency_tests" in concur_result
                else {}
            )
            logger.info(
                f"并发优化: 最佳QPS {best_result.get('qps', 0):.2f}, 最佳并发数 {best_result.get('concurrency', 0)}"
            )
        ...
        if "image_processing_optimization" in self.test_results:
            ...
            logger.info(
                f"图片处理优化: 质量评估耗时 {img_result.get('quality_time', 0):.4f}秒, 去重后剩余 {img_result.get('deduplicate_count', 0)}张"
            )
        ...
        if "data_collection_optimization" in self.test_results:
            ...
            logger.info(
                f"数据采集优化: 采集 {coll_result.get('collected_images', 0)}张图片, 耗时 {coll_result.get('collection_time', 0):.4f}秒"
            )
