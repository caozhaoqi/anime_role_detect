...
class OptimizationTester:
    ...
    def __init__(self, test_output_dir="data/test_optimization"):
        ...
        self.test_results = {
            ...
            "end_time": None,
            "duration": 0,
            "network_stats": {"total_requests": 0, "successful_requests": 0, "failed_requests": 0},
            "concurrency_stats": {"initial_workers": 0, "max_workers": 0, "avg_workers": 0},
            "image_stats": {
                "total_images": 0,
                "successful_images": 0,
                "failed_images": 0,
        ...
        try:
            ...
            for i, img in enumerate(images[:max_images]):
                save_path = os.path.join(
                    self.test_output_dir, f"{test_series}_{test_character}_{i:04d}.jpg"
                )
                future = collector.concurrency_manager.submit(
                    collector._download_image, img["url"], save_path
                )
            ...
            logger.info(
                f"并发处理性能测试完成，耗时: {duration:.2f} 秒，下载 {downloaded_count}/{len(images)} 张图片"
            )
        except Exception as e:
    ...
    def test_image_processing(self):
        ...
        try:
            ...
            for i, img in enumerate(images[:max_images]):
                save_path = os.path.join(
                    self.test_output_dir, f"{test_series}_{test_query}_{i:04d}.jpg"
                )
            ...
            self.test_results["image_stats"].update(
                {
                    ...
                    "avg_processing_time": (
                        total_processing_time / len(images) if len(images) > 0 else 0
                    ),
                }
            )

            logger.info(
                f"图片处理性能测试完成，耗时: {duration:.2f} 秒，成功处理 {successful_images}/{len(images)} 张图片"
            )
        except Exception as e:
    ...
    def test_overall_performance(self):
        ...
        try:
            ...
            self.test_results["overall_stats"] = {
                ...
                "avg_images_per_character": (
                    total_images / total_characters if total_characters > 0 else 0
                ),
                "avg_time_per_character": (
                    duration / total_characters if total_characters > 0 else 0
                ),
            }

            logger.info(
                f"整体性能测试完成，耗时: {duration:.2f} 秒，采集 {total_characters} 个角色，共 {total_images} 张图片"
            )
    ...
    def run_all_tests(self):
        ...
        self.test_results["duration"] = (
            time.time() - datetime.fromisoformat(self.test_results["start_time"]).timestamp()
        )

        # 保存测试结果
        test_result_file = os.path.join(
            self.test_output_dir, f'test_result_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
        )
    ...
    def generate_test_report(self):
        ...
        report_file = os.path.join(
            self.test_output_dir, f'test_report_{datetime.now().strftime("%Y%m%d_%H%M%S")}.md'
        )
...
def main():
    ...
    print(
        f"网络请求成功率: {((test_results['network_stats'].get('successful_requests', 0) / max(test_results['network_stats'].get('total_requests', 1), 1)) * 100):.2f}%"
    )
    print(
        f"并发处理成功率: {((test_results['concurrency_stats'].get('downloaded_images', 0) / max(test_results['concurrency_stats'].get('total_images', 1), 1)) * 100):.2f}%"
    )
    print(
        f"图片处理成功率: {((test_results['image_stats'].get('successful_images', 0) / max(test_results['image_stats'].get('total_images', 1), 1)) * 100):.2f}%"
    )
    print(
        f"错误处理错误率: {((test_results['error_stats'].get('total_errors', 0) / max(test_results['error_stats'].get('test_cases', 1), 1)) * 100):.2f}%"
    )
    print(
        f"整体性能: 处理 {test_results.get('overall_stats', {}).get('total_characters', 0)} 个角色，{test_results.get('overall_stats', {}).get('total_images', 0)} 张图片"
    )
    print("=" * 80)
