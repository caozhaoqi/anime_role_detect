    ...
    if model:
        for img_path in test_images:
            try:
                ...
                logger.info(
                    f"PyTorch推理时间: {inference_time:.2f}ms - {os.path.basename(img_path)}"
                )
            except Exception as e:
...
def main():
    ...
    parser.add_argument(
        "--test_dir", type=str, default="data/split_dataset/val", help="测试图像目录"
    )
    parser.add_argument("--num_images", type=int, default=10, help="测试图像数量")
    parser.add_argument(
        "--input_size", type=int, nargs=2, default=[224, 224], help="输入尺寸 (height width)"
    )
