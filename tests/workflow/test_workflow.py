...
class WorkflowTest:
    ...
    def test_data_preprocessing(self):
        ...
        for character_dir in os.listdir(self.output_dir):
            ...
            if os.path.isdir(character_path):
                ...
                logger.info(
                    f"角色 {character_dir} 预处理完成，平均质量: {preprocessing_results[character_dir]['avg_quality']:.2f}"
                )

        result = {"preprocessing_results": preprocessing_results}
    ...
    def test_data_augmentation(self):
        ...
        result = {"augmentation_results": augmentation_results}
    ...
    def test_dataset_organization(self):
        ...
        for character_dir in os.listdir(self.output_dir):
            ...
            if os.path.isdir(character_path):
                ...
                logger.info(
                    f"角色 {character_dir} 组织完成，训练集: {len(train_images)}, 验证集: {len(val_images)}"
                )
    ...
    def print_test_summary(self):
        ...
        if "data_collection" in self.test_results:
            ...
            logger.info(
                f"数据采集: 总角色数 {stats.get('total_characters', 0)}, 总图片数 {stats.get('total_images', 0)}"
            )
        ...
        if "data_preprocessing" in self.test_results:
            ...
            logger.info(
                f"数据预处理: 已处理 {len(prep_result.get('preprocessing_results', {}))} 个角色"
            )
        ...
        if "data_augmentation" in self.test_results:
            ...
            total_aug = sum(
                r["augmented_images"] for r in aug_result.get("augmentation_results", {}).values()
            )
        ...
        if "dataset_organization" in self.test_results:
            ...
            total_train = sum(
                r["train_images"] for r in org_result.get("organization_results", {}).values()
            )
            total_val = sum(
                r["val_images"] for r in org_result.get("organization_results", {}).values()
            )
