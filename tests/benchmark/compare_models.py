    ...
    for i, (image_path, expected_role) in enumerate(test_data):
        ...
        try:
            ...
            predicted_role, similarity, boxes = classifier.classify_image(
                image_path, use_model=use_model
            )
...
def test_model_performance(classifier, test_image_path, iterations=10):
    ...
    return {"clip": clip_stats, "efficientnet": efficientnet_stats}


def generate_model_comparison_report(
    results_list, performance_results, output_dir="model_comparison_reports"
):
    """生成模型对比报告
    ...
    with open(report_md_file, "w", encoding="utf-8") as f:
        ...
        f.write("## 准确率对比\n\n")
        f.write(
            "| 模型 | 测试总数 | 正确预测 | 错误预测 | 准确率 | 平均推理时间 (ms) | 误检数量 |\n"
        )
        f.write(
            "|------|----------|----------|----------|--------|------------------|----------|\n"
        )
        ...
        f.write(
            f"- **准确率最高的模型**: {best_accuracy_model['model_name']} ({best_accuracy_model['accuracy']:.2f}%)\n"
        )

        # 找出速度最快的模型
        best_speed_model = min(results_list, key=lambda x: x["average_inference_time"])
        f.write(
            f"- **速度最快的模型**: {best_speed_model['model_name']} ({best_speed_model['average_inference_time']:.2f}ms)\n"
        )
...
def main():
    ...
    parser.add_argument(
        "--max_images_per_class", type=int, default=10, help="每个类别的最大测试图像数量"
    )
    parser.add_argument("--iterations", type=int, default=10, help="性能测试迭代次数")
    parser.add_argument(
        "--output_dir", type=str, default="model_comparison_reports", help="测试报告输出目录"
    )
    ...
    # 2. 测试EfficientNet模型
    try:
        efficientnet_results = test_model_accuracy(
            classifier, test_data, use_model=True, model_name="EfficientNet"
        )
    ...
    # 生成模型对比报告
    report_file, report_md_file = generate_model_comparison_report(
