...
def benchmark_accuracy(classifier, test_image_path, expected_role, iterations=5):
    ...
    for i in range(iterations):
        ...
        predicted_role, similarity, boxes = classifier.classify_image(
            test_image_path, use_model=False
        )
...
def benchmark_input_sizes(
    classifier,
    test_image_path,
    input_sizes=[(224, 224), (256, 256), (384, 384), (512, 512)],
    iterations=3,
):
    """基准测试不同输入大小的性能
...
def generate_benchmark_report(benchmark_results, output_dir="benchmark_reports"):
    ...
    with open(report_md_file, "w", encoding="utf-8") as f:
        ...
        if "inference_speed" in benchmark_results:
            f.write("## 推理速度测试\n\n")
            f.write(
                "| 批量大小 | 迭代次数 | 平均时间 (秒) | 标准差 (秒) | 最小时间 (秒) | 最大时间 (秒) | FPS | 每张图像时间 (秒) |\n"
            )
            f.write(
                "|---------|---------|--------------|------------|--------------|--------------|-----|------------------|\n"
            )
            ...
            for result in benchmark_results["inference_speed"]:
                f.write(
                    f"| {result['batch_size']} | {result['iterations']} | {result['average_time']:.4f} | {result['std_time']:.4f} | {result['min_time']:.4f} | {result['max_time']:.4f} | {result['fps']:.2f} | {result['per_image_time']:.4f} |\n"
                )
        ...
        if "input_sizes" in benchmark_results:
            ...
            for result in benchmark_results["input_sizes"]:
                f.write(
                    f"| {result['input_size']} | {result['iterations']} | {result['average_time']:.4f} | {result['std_time']:.4f} |\n"
                )
...
def main():
    ...
    parser.add_argument(
        "--input_sizes", type=str, default="224x224,256x256,384x384", help="输入大小列表，逗号分隔"
    )
    parser.add_argument(
        "--output_dir", type=str, default="benchmark_reports", help="基准测试报告输出目录"
    )
