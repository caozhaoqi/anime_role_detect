...
def prepare_benchmark_data(target_roles, num_images_per_role=5, output_dir="tests/benchmark_data"):
    ...
    for role in target_roles:
        ...
        # 检查现有图片数量
        existing_images = [
            f for f in os.listdir(role_dir) if f.lower().endswith((".jpg", ".png", ".jpeg"))
        ]
        ...
        for img_path in images:
            dataset_info.append({"image_path": img_path, "true_label": role})
...
def generate_report(results, output_dir="reports"):
    ...
    for model in ["clip", "efficientnet"]:
        ...
        metrics[model] = {"accuracy": acc, "avg_time": avg_time, "fps": fps}
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        f.write("| :--- | :---: | :---: | :---: |\n")
        f.write(
            f"| **CLIP + FAISS** | {metrics['clip']['accuracy']:.2%} | {metrics['clip']['avg_time']:.2f} | {metrics['clip']['fps']:.2f} |\n"
        )
        f.write(
            f"| **EfficientNet** | {metrics['efficientnet']['accuracy']:.2%} | {metrics['efficientnet']['avg_time']:.2f} | {metrics['efficientnet']['fps']:.2f} |\n\n"
        )
        ...
        for model in ["clip", "efficientnet"]:
            ...
            f.write(
                classification_report(
                    results[model]["true"], results[model]["pred"], zero_division=0
                )
            )
            f.write("```\n\n")
...
def main():
    ...
    parser.add_argument(
        "--roles",
        nargs="+",
        default=["原神_胡桃", "原神_雷泽", "Re0_雷姆", "初音未来"],
        help="测试角色列表",
    )
