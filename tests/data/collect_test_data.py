        ...
        for illust in json_result.illusts:
            ...
            if illust.page_count == 1:
                ...
                for page in illust.meta_pages:
                    ...
                    print(
                        f"准备下载: {illust.title} (p{page.image_urls.original.split('_p')[-1].split('.')[0]}) - {image_url}"
                    )
    ...
    try:
        ...
        if api_key and user:
            headers["Authorization"] = (
                f"Basic {base64.b64encode(f'{user}:{api_key}'.encode()).decode()}"
            )
...
def collect_single_character_data(
    character_name, limit, output_dir, refresh_token=None, api_key=None, user=None
):
    """采集单角色数据"""
...
def main():
    ...
    parser.add_argument(
        "--character", required=True, help="角色名称 (例如: 'rem_(re:zero)', 'genshin_impact klee')"
    )
    parser.add_argument("--limit", type=int, default=5, help="采集图片数量")
    parser.add_argument(
        "--output_dir",
        help="输出目录 (默认为 'tests/test_images/single_character/<character_name>')",
    )

    # API认证参数
    parser.add_argument(
        "--refresh_token", help="Pixiv API的refresh_token。强烈建议使用环境变量 PIXIV_REFRESH_TOKEN"
    )
    ...
    output_dir = args.output_dir or os.path.join(
        "tests/test_images/single_character", args.character.replace(" ", "_").replace(":", "_")
    )

    # 执行采集
    collect_single_character_data(
        args.character, args.limit, output_dir, refresh_token, args.api_key, args.user
    )
