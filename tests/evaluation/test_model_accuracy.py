...
def test_model_accuracy(data_dir, model_name=""):
    ...
    for img_path, true_role in test_data:
        try:
            # 使用集成分类方法
            predicted_role, similarity, boxes = classifier.classify_image_with_deepdanbooru(
                img_path
            )
            ...
            logger.info(
                f"图片: {os.path.basename(img_path)}, 真实角色: {true_role}, 预测角色: {predicted_role}, 相似度: {similarity:.4f}, {'正确' if is_correct else '错误'}"
            )
