            ...
            for img_file in image_files:
                ...
                try:
                    ...
                    if predicted_role == "unknown" or similarity < self.threshold:
                        ...
                        print(
                            f"  图片 {img_file}: 无法识别 (相似度: {similarity:.4f}, 时间: {processing_time:.4f}s)"
                        )
                    elif predicted_role == role:
                        self.results["single_character"]["correct"] += 1
                        print(
                            f"  图片 {img_file}: 正确识别为 {predicted_role} (相似度: {similarity:.4f}, 时间: {processing_time:.4f}s)"
                        )
                    else:
                        self.results["single_character"]["incorrect"] += 1
                        print(
                            f"  图片 {img_file}: 错误识别为 {predicted_role} (相似度: {similarity:.4f}, 时间: {processing_time:.4f}s)"
                        )
            ...
            try:
                ...
                if characters:
                    ...
                    characters_with_features = (
                        self.extractor.extract_features_from_multiple_characters(characters)
                    )

                    # 分类
                    classified_characters = self.classifier.classify_multiple_characters(
                        characters_with_features
                    )
                    ...
                    for i, char in enumerate(classified_characters):
                        ...
                        if role == "unknown" or similarity < self.threshold:
                            ...
                            print(
                                f"  角色{i+1}: 无法识别 (检测置信度: {confidence:.4f}, 相似度: {similarity:.4f})"
                            )
                        else:
                            # 这里简化处理，实际应用中需要标注多角色的真实标签
                            self.results["multiple_characters"]["correct"] += 1
                            print(
                                f"  角色{i+1}: {role} (检测置信度: {confidence:.4f}, 相似度: {similarity:.4f})"
                            )
                else:
                    end_time = time.time()
                    processing_time = end_time - start_time
    ...
    def generate_report(self, report_path="tests/test_results/evaluation_report.txt"):
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            ...
            if multi_results["total"] > 0:
                avg_characters_per_image = (
                    multi_results["correct"] + multi_results["unknown"]
                ) / multi_results["total"]
...
def main():
    ...
    parser.add_argument(
        "--single_character_dir",
        default="tests/test_images/single_character",
        help="单角色测试数据目录",
    )
    parser.add_argument(
        "--multiple_character_dir",
        default="tests/test_images/multiple_characters",
        help="多角色测试数据目录",
    )
