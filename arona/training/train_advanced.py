...
# -*- coding: utf-8 -*-
"""
高级训练脚本 - 实施Mixup、CutMix、标签平滑等高级技术
"""
...
def train_model(model, train_loader, val_loader, device, num_epochs=50, lr=0.001, weight_decay=0.0005, 
                output_dir='models/advanced', class_to_idx=None, use_mixup=True, use_cutmix=False, 
                label_smoothing=0.1):
    logger.info(f"开始训练，设备: {device}")
    logger.info(f"数据增强: Mixup={use_mixup}, CutMix={use_cutmix}, 标签平滑={label_smoothing}")
    ...
    results = {
        'model_type': 'mobilenet_v2_advanced',
...
def main():
    ...
    parser.add_argument('--use-cutmix', action='store_true', help='使用CutMix数据增强')
    parser.add_argument('--output-dir', type=str, default='../../models/advanced', help='输出目录')
    ...
    target_characters = ['arona', 'plana', 'hina']
