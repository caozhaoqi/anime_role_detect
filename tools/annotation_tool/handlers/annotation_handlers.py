...
class AnnotationHandler:
        ...
        try:
            ...
            if old_index < len(self.main_window.images):
                ...
                for i, img_data in enumerate(self.main_window.images):
                    if img_data.get('folder', '') == old_folder and i < len(self.main_window.images):
                        new_index = min(i, len(self.main_window.images) - 1)
                        break

            if new_index < 0:
                new_index = max(0, len(self.main_window.images) - 1) if self.main_window.images else 0
    ...
    def delete_image_at_index(self, idx):
        ...
        reply = QMessageBox.question(self.main_window, "确认", f"确定要删除图片吗？\n{img['filename']}")
    ...
    def delete_role(self):
        ...
        if current_row >= 0 and current_row < len(self.main_window.roles):
            ...
            reply = QMessageBox.question(self.main_window, "确认", f"确定要删除角色 {role.name} 吗？")
            if reply == QMessageBox.Yes:
