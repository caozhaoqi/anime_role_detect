...
class ZoomableDragableLabel(QLabel):
    ...
    def _log_event(self, event_name, event):
        ...
        pos = event.pos()
        logger.debug(
            f"[Event#{event_name}] {event_name}: btn={btn_str}, pos=({pos.x()},{pos.y()}), size={self.width()}x{self.height()}"
        )
    ...
    def wheelEvent(self, event):
        ...
        if self.zoom_level != old_zoom:
            logger.info(
                f"[Event#] wheelEvent: delta={delta}, zoom_before={old_zoom}, zoom_after={self.zoom_level}"
            )
            self.update()
    ...
    def mouseMoveEvent(self, event):
        ...
        if hasattr(self, "_log_event"):
            ...
            if move_pos:
                logger.debug(
                    f"[Event#] mouseMoveEvent: btn=0, pos=({move_pos.x()},{move_pos.y()}), label_size={label_size[0]}x{label_size[1]}, parent_size={parent_size[0]}x{parent_size[1]}, is_panning={self.is_panning}, zoom={zoom}"
                )
...
class ClickableLabel(ZoomableDragableLabel):
            ...
            if main_window:
                ...
                if clicked_idx is not None:
                    if event.modifiers() & Qt.ControlModifier:
                        ...
                        logger.info(
                            f"[Selection] Ctrl+click: selected_indices={main_window.selected_indices}"
                        )
                        main_window.image_handler.show_current_image()
                        return
                    elif event.modifiers() & Qt.ShiftModifier:
                        if (
                            main_window.current_index in main_window.selected_indices
                            or not main_window.selected_indices
                        ):
                            main_window.selected_indices.add(clicked_idx)
                        else:
                        ...
                        logger.info(
                            f"[Selection] Shift+click: selected_indices={main_window.selected_indices}"
                        )
                        main_window.image_handler.show_current_image()
                        return
                    else:
                        ...
                        main_window.selected_indices.add(clicked_idx)
                        logger.info(
                            f"[Selection] Click: selected_indices={main_window.selected_indices}"
                        )
                        main_window.image_handler.show_current_image()
    ...
    def get_clicked_grid_index(self, pos):
        ...
        logger.info(
            f"[GridClick] pos={pos}, label={label_width}x{label_height}, cell={cell_width}x{cell_height}, row={row}, col={col}, cell_index={cell_index}, grid_start={self.grid_start_index}, clicked_idx={clicked_idx}"
        )
    ...
    def show_context_menu(self, pos):
        ...
        logger.info(
            f"[Menu] clicked_idx from get_clicked_grid_index: {clicked_idx}, type: {type(clicked_idx)}"
        )
        if clicked_idx is None or clicked_idx >= len(main_window.images):
            logger.info(
                f"[Menu] Invalid clicked index: {clicked_idx}, total images: {len(main_window.images)}"
            )
            return
        ...
        delete_action.triggered.connect(
            functools.partial(main_window.delete_image_at_index, clicked_idx)
        )
        menu.addAction(delete_action)

        move_r18_action = QAction(f"移动到R18 ({clicked_idx + 1})", self)
        move_r18_action.triggered.connect(
            functools.partial(main_window.move_image_at_index, clicked_idx, "R18")
        )
        menu.addAction(move_r18_action)

        move_multi_action = QAction(f"移动到多角色 ({clicked_idx + 1})", self)
        move_multi_action.triggered.connect(
            functools.partial(main_window.move_image_at_index, clicked_idx, "多角色")
        )
        menu.addAction(move_multi_action)

        move_other_action = QAction(f"移动到其他 ({clicked_idx + 1})", self)
        move_other_action.triggered.connect(
            functools.partial(main_window.move_image_at_index, clicked_idx, "其他")
        )
