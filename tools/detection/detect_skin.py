...
def process_dataset(dataset_path, output_path, use_anime_mode=True, sample_limit=None):
    ...
    for i, img_path in enumerate(image_files):
        path, skin_percent, num_regions, skin_pixels, has_skin = analyze_skin(img_path, use_anime_mode)
        ...
        if (i + 1) % 500 == 0:
            print(f"已处理: {i + 1}/{total} | 有皮肤: {len(skin_images)} | 无皮肤: {len(no_skin_images)}")
    ...
    if skin_images:
        ...
        for path, skin_percent, num_regions in sorted(skin_images, key=lambda x: x[1], reverse=True):
            src = Path(path)
