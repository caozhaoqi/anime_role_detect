...
def process_dataset(dataset_path, output_path, min_faces=2, sample_limit=None):
    ...
    for i, img_path in enumerate(image_files):
        ...
        if (i + 1) % 500 == 0:
            ...
            print(f"已处理: {i + 1}/{total} | 多脸: {len(multi_face_images)} | 单脸: {len(single_face_images)} | 无脸: {len(no_face_images)} | 灰度: {gray_count}")

    print(f"\n处理完成!")
    print(f"=" * 70)
    print(f"总图片数: {len(image_files)}")
    print(f"多角色图片 (>=2人脸): {len(multi_face_images)} ({len(multi_face_images)/len(image_files)*100:.1f}%)")
    print(f"单角色图片 (=1人脸): {len(single_face_images)} ({len(single_face_images)/len(image_files)*100:.1f}%)")
    print(f"无检测到人脸: {len(no_face_images)} ({len(no_face_images)/len(image_files)*100:.1f}%)")
    print(f"-" * 70)
    print(f"灰度/黑白图片: {len(grayscale_images)} ({len(grayscale_images)/len(image_files)*100:.1f}%)")
    ...
    if multi_face_images:
        print(f"\n正在复制多角色图片...")
        gray_multi = 0
        for img_path, face_count, is_gray in sorted(multi_face_images, key=lambda x: x[1], reverse=True):
            src = Path(img_path)
