...
*   **核心功能**:
    *   **多源数据采集**: 支持从 Safebooru、Pixiv、Danbooru 等多个源自动收集角色图片。
    *   **预设角色列表**: 内置多个系列的角色列表，包括原神、崩坏、鬼灭之刃、火影忍者和海贼王等。
    *   **图像验证**: 自动验证下载的图像是否有效，过滤低质量图像。
    *   **数据集统计**: 提供详细的数据集统计信息，包括每个角色的图像数量和质量分布。
*   **技术实现**:
    *   使用 `requests` 库发送 HTTP 请求获取图像，支持代理和重试机制。
    *   使用 `PIL` 库验证图像有效性和质量。
    *   多线程并发下载，提高数据采集速度。
    *   集成图像质量评估算法，自动过滤低质量图像。

### 2.6 模型蒸馏 (Model Distillation)
*   **实现文件**: `scripts/model_training/train_model_distillation.py`
*   **架构设计**: **Teacher-Student** 架构，通过知识迁移实现模型压缩。
*   **教师模型**: 使用 **EfficientNet-B0** 作为教师模型，提供丰富的特征表示和准确的概率分布。
...
*   **核心功能**:
    ...
    *   **自适应学习**: 根据新数据的分布自动调整模型参数。
    *   **模型评估**: 定期评估模型性能，确保在线学习的有效性。
*   **技术实现**:
...
*   **架构设计**: **双编码器 + 融合层** 架构，整合图像和文本信息以提高识别精度。
*   **图像编码器**: 使用 **EfficientNet-B0** 提取图像特征，捕捉视觉信息。
*   **文本编码器**: 使用 **BERT** 处理文本描述，提取语义信息。
*   **融合方法**: 支持多种融合方法，包括:
    *   **Concat**: 拼接图像和文本特征，充分利用两种模态的信息。
    *   **Add**: 对图像和文本特征进行逐元素相加，捕捉两种模态的共同信息。
    *   **Multiply**: 对图像和文本特征进行逐元素相乘，增强两种模态的相关信息。
    *   **Attention**: 使用注意力机制动态调整两种模态的权重，突出重要信息。
*   **分类器**: 多层全连接网络，用于最终的角色分类。
*   **训练策略**:
    *   联合训练图像和文本编码器。
    *   使用对比损失和分类损失的组合。
    *   采用渐进式训练策略，先训练编码器，再训练融合层。
*   **优势**:
    *   提高识别准确率，特别是对于相似角色的区分。
    *   增强系统的鲁棒性，减少对单一模态的依赖。
    *   支持基于文本描述的角色检索。
