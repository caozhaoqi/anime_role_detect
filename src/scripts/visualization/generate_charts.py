...
# 将性能转换为评分（5分制）
speed_scores = [
    5 - (x - min(inference_speed)) / (max(inference_speed) - min(inference_speed)) * 4
    for x in inference_speed
]
size_scores = [
    5 - (x - min(model_size)) / (max(model_size) - min(model_size)) * 4 for x in model_size
]
...
# 归一化数据
normalized_speed = [
    1 - (x - min(inference_speed)) / (max(inference_speed) - min(inference_speed))
    for x in inference_speed
]
normalized_size = [
    1 - (x - min(model_size)) / (max(model_size) - min(model_size)) for x in model_size
]
