...
class LLMClient:
    ...
    def get_game_characters(self, game_name):
        ...
        for i, prompt in enumerate(prompts):
            ...
            for retry in range(3):
                try:
                    ...
                    response = self.call_llm(prompt, system_prompt, max_tokens=1000, temperature=0.7)
        ...
        # 过滤掉空字符串和无效角色名
        characters = [c for c in characters if c and len(c) > 1 and not any(keyword in c for keyword in ['角色', '包括', '中的', '等', '游戏'])]
