...
class ClassificationScript:
    ...
    def process_multiple_characters(self, image_path):
        ...
        try:
            ...
            # 提取特征
            characters_with_features = self.extractor.extract_features_from_multiple_characters(
    ...
    def classify_and_archive(self):
        ...
        for i, image_path in enumerate(image_files):
            ...
            if characters:
                ...
                print(
                    f"  检测到 {len(characters)} 个角色，选择置信度最高的: {role} (相似度: {similarity:.4f})"
                )
            else:
                # 回退到单角色处理
    ...
    def correct_classification(self, image_path, correct_role):
        ...
        if success:
            ...
            print(
                f"分类结果已修正，索引已更新: 图像 {image_path} 已添加到角色 {correct_role} 的索引中"
            )
        else:
...
if __name__ == "__main__":
    ...
    # 验证索引文件
    if not os.path.exists(f"{args.index}.faiss") or not os.path.exists(
        f"{args.index}_mapping.json"
    ):
