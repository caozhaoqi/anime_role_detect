...
class PerformanceTester:
    ...
    def test_response_time(self):
        ...
        for img_path, true_role in self.test_images[:5]:  # 只测试前5张图片
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        f"图片 {os.path.basename(img_path)} 响应时间: {response_time:.2f} 秒"
                    )
                else:
        ...
        if response_times:
            ...
            logger.info(
                f"响应时间统计: 平均 {avg_time:.2f} 秒, 最大 {max_time:.2f} 秒, 最小 {min_time:.2f} 秒"
            )
    ...
    def test_resource_usage(self):
        ...
        logger.info(
            f"内存使用从 {before_memory:.2f} MB 变为 {after_memory:.2f} MB，增加了 {resource_usage['memory_increase']:.2f} MB"
        )
        ...
        for img_path, true_role in self.test_images[:10]:  # 只测试前10张图片
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        f"图片 {os.path.basename(img_path)}: 真实角色={true_role}, 预测角色={predicted_role}, AI预测={ai_predicted_role}, 相似度={similarity:.4f}, {'正确' if is_correct else '错误'}"
                    )
                else:
                    logger.error(f"API请求失败，状态码: {response.status_code}")
            except Exception as e:
    ...
    def test_tagging_quality(self):
        ...
        tag_stats = {"total_tags": 0, "total_images": 0, "avg_tags_per_image": 0}
        ...
        if tag_stats["total_images"] > 0:
            ...
            logger.info(
                f"标签生成统计: 平均每张图片生成 {tag_stats['avg_tags_per_image']:.2f} 个标签"
            )
...
def main():
    ...
    logger.info(
        f"CPU 使用率: 从 {resource_usage['cpu_before']:.2f}% 变为 {resource_usage['cpu_after']:.2f}%"
    )
    logger.info(
        f"内存使用: 从 {resource_usage['memory_before']:.2f} MB 变为 {resource_usage['memory_after']:.2f} MB"
    )
