...
def main():
    ...
    parser.add_argument("--manager-id", type=str, default="manager_001", help="管理器ID")
    parser.add_argument("--queue-path", type=str, default="./task_queue", help="任务队列存储路径")
    parser.add_argument("--max-workers", type=int, default=5, help="最大工作线程数")

    # 添加任务相关参数
    parser.add_argument("--series", type=str, help="系列名称")
    parser.add_argument("--keyword-file", type=str, help="关键词文件路径")
    parser.add_argument("--characters", type=str, nargs="+", help="角色列表")
    parser.add_argument("--max-images", type=int, default=50, help="每个角色的最大图像数")
    ...
    manager = DistributedManager(
        manager_id=args.manager_id, queue_path=args.queue_path, max_workers=args.max_workers
    )
    ...
    if args.action == "start":
        ...
        task_ids = manager.add_image_collection_tasks(
            series=args.series, characters=characters, max_images=args.max_images
        )
