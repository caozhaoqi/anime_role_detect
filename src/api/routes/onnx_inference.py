...
async def predict(model_name: str, file: UploadFile = File(...), use_gpu: Optional[bool] = False):
    """
    使用指定模型进行单张图片推理
...
async def predict_batch(
    model_name: str, files: List[UploadFile] = File(...), use_gpu: Optional[bool] = False
):
    """
    使用指定模型进行批量图片推理
    ...
    try:
        ...
        # 记录指标
        record_metrics(
            endpoint="/api/v1/onnx/predict/batch",
            method="POST",
            status=200,
            model_name=model_name,
            duration=duration,
