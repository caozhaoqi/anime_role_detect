...
class MemoryMonitor:
    ...
    def _check_memory_usage(self, memory_percent, memory_used, memory_total):
        ...
        if (
            ...
            and current_time - self.last_alert_time > self.cooldown_period
        ):
            logger.warning(
                f"内存使用预警: {memory_percent:.2f}% (已使用: {memory_used:.2f}GB, 总内存: {memory_total:.2f}GB)"
            )
    ...
    def _check_memory_leak(self):
        ...
        if older_values and recent_values:
            ...
            if avg_recent > avg_older + 3:
                logger.warning(
                    f"检测到内存泄漏迹象: 内存使用从 {avg_older:.2f}% 增长到 {avg_recent:.2f}%"
                )
                # 执行垃圾回收
                import gc
    ...
    def _generate_coredump(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["gcore", "-o", coredump_file, str(current_pid)], capture_output=True, text=True
            )
        ...
        try:
            ...
            for key in ["total", "available", "used", "free"]:
                if key in memory_dump["memory"]:
                    memory_dump["memory"][f"{key}_gb"] = memory_dump["memory"][key] / (
                        1024 * 1024 * 1024
                    )

            for key in ["total", "used", "free"]:
                if key in memory_dump["swap"]:
                    memory_dump["swap"][f"{key}_gb"] = memory_dump["swap"][key] / (
                        1024 * 1024 * 1024
                    )
        ...
        try:
            ...
            for proc in psutil.process_iter(["pid", "name", "memory_info", "memory_percent"]):
                try:
                    ...
                    # 检查memory_percent是否为None
                    memory_percent = proc_info.get("memory_percent")
                    if proc_info["pid"] == current_pid or (
                        memory_percent is not None and memory_percent > 0.5
                    ):
    ...
    def _log_emergency(self):
        ...
        try:
            ...
            emergency_file = os.path.join(
                self.memory_monitor.save_dir, f"emergency_{timestamp}.log"
            )
