...
class DataSource(ABC):
    ...
    def calculate_quality_score(self):
        ...
        response_time_factor = max(
            0, 1 - (self.stats["avg_response_time"] / 5)
        )  # 5秒以上响应时间视为差
        failure_factor = max(
            0, 1 - (self.stats["consecutive_failures"] / 10)
        )  # 连续失败10次以上视为差
    ...
    def fetch_images(self, query, limit=50):
        ...
        try:
            for _ in range(min(limit, 10)):  # 每次最多获取10张
                ...
                if data.get("url"):
                    images.append(
                        {"url": data["url"], "width": 0, "height": 0, "source": self.name}
                    )
...
class DanbooruDataSource(DataSource):
    ...
    def fetch_images(self, query, limit=50):
        ...
        try:
            ...
            params = {"tags": query, "limit": limit, "random": "true"}
    ...
    def get_data_source_stats(self):
        ...
        for name, source in self.data_sources.items():
            ...
            stats[name] = {
                ...
                "success_rate": (
                    source.stats["successful_requests"] / source.stats["total_requests"]
                    if source.stats["total_requests"] > 0
                    else 0
                ),
