...
class Task(ABC):
    ...
    def from_dict(cls, task_dict):
        ...
        """
        task = cls(
            task_id=task_dict["task_id"], task_type=task_dict["task_type"], **task_dict["params"]
        )
        ...
        task.start_time = (
            datetime.fromisoformat(task_dict["start_time"]) if task_dict["start_time"] else None
        )
        task.end_time = (
            datetime.fromisoformat(task_dict["end_time"]) if task_dict["end_time"] else None
        )
    ...
    def execute(self):
        ...
        try:
            ...
            with Image.open(self.params["image_path"]) as img:
                ...
                return {"role": role, "similarity": float(similarity)}
    ...
    def execute(self):
        ...
        try:
            ...
            result = collector._process_character(
                series=self.series, character_name=self.character, max_images=self.max_images
            )
...
class TaskQueue:
    ...
    def add_task(self, task):
        ...
        with self.lock:
            ...
            logger.info(
                f"添加任务: {task.task_id}, 类型: {task.task_type}, 优先级: {task.priority}"
            )
...
class DistributedManager:
    ...
    def add_task(self, task_type, **kwargs):
        ...
        if task_type == "image_collection":
            task = ImageCollectionTask(task_id=task_id, task_type=task_type, **kwargs)
        else:
    ...
    def add_image_collection_tasks(self, series, characters, max_images=50):
        ...
        for character in characters:
            task_id = self.add_task(
                "image_collection", series=series, character=character, max_images=max_images
            )
