...
class LogManager:
    ...
    def __init__(
        self, log_dir="logs", log_level=logging.INFO, max_bytes=10 * 1024 * 1024, backup_count=5
    ):
        """
        初始化日志管理器
    ...
    def _configure_root_logger(self):
        ...
        log_file = os.path.join(
            self.log_dir, f'application_{datetime.now().strftime("%Y%m%d")}.log'
        )
    ...
    def log_collection_stats(self, collector_name, stats):
        ...
        stats_file = os.path.join(
            self.log_dir, f'collection_stats_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
        )
    ...
    def analyze_logs(self, log_file=None, days=7):
        ...
        analysis_result = {
            "total_logs": 0,
            "level_distribution": {"INFO": 0, "WARNING": 0, "ERROR": 0, "CRITICAL": 0},
            "top_errors": [],
            "time_distribution": {},
            "duration": 0,
        }
                ...
                with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
                    for line in f:
                        ...
                        if " - " in line:
                            ...
                            analysis_result["time_distribution"][hour] = (
                                analysis_result["time_distribution"].get(hour, 0) + 1
                            )
            except Exception as e:
        ...
        for error_msg, count in sorted_errors:
            analysis_result["top_errors"].append({"message": error_msg.strip(), "count": count})

        analysis_result["duration"] = time.time() - start_time

        # 保存分析结果
        analysis_file = os.path.join(
            self.log_dir, f'log_analysis_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
        )
        try:
