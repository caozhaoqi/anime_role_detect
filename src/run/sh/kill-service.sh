...
if command -v supervisorctl &> /dev/null; then
    ...
    supervisorctl -s http://127.0.0.1:9001 -u admin -p admin123 shutdown 2>/dev/null || true
    sleep 3
fi

# 2. 检查并强制关闭 supervisord 主进程
SUPERVISOR_PIDS=$(pgrep -f "supervisord" 2>/dev/null || true)
# 修复 -not -z 语法错误，改用标准的 -n (非空) 判断
if [ -n "$SUPERVISOR_PIDS" ] || pgrep -x "supervisord" &>/dev/null; then
    echo "[INFO] 发现 supervisord 残留进程，正在强制终止..."
    # 尝试优雅终止
    pkill -15 -f "supervisord" 2>/dev/null || true
    sleep 1
    # 强制终止
...
for PORT in "${PORTS[@]}"; do
    # 使用 lsof 查找占用端口的 PID，追加 || true 确保无占用时命令不报错
    PIDS=$(lsof -t -i :$PORT 2>/dev/null || true)
    if [ -n "$PIDS" ]; then
        echo "[WARN] 端口 $PORT 仍被 PID: $PIDS 占用，正在强制释放..."
        # 兼容可能有多个 PID 的场景，循环逐个杀掉
        for pid in $PIDS; do
...
NODE_PIDS=$(pgrep -f "next-server|node.*frontend" 2>/dev/null || true)
if [ -n "$NODE_PIDS" ]; then
