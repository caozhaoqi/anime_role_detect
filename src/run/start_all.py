        ...
        for key, config in SERVICES.items():
            if processes.get(key) and processes[key].poll() is None:
                try:
                    ...
                    response = requests.get(f"http://localhost:{config['port']}{health_path}", timeout=2)
...
def print_api_docs():
    ...
    api_categories = {
        "搜索服务 API": {
            ...
            "endpoints": [
                {"method": "POST", "path": "/search/image", "description": "以图搜图，上传图像搜索相似角色"},
                {"method": "POST", "path": "/search/build-index", "description": "构建FAISS搜索索引"},
                {"method": "GET", "path": "/search/stats", "description": "获取索引统计信息"},
                {"method": "POST", "path": "/search/video/recognize", "description": "视频实时抽帧识别"},
        ...
        "主API服务 API": {
            ...
            "endpoints": [
                ...
                {"method": "POST", "path": "/api/search/video/recognize", "description": "视频识别"},
...
def main():
    ...
    parser.add_argument("--group", "-g", default="all", 
