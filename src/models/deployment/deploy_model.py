    ...
    def predict(self, image_content, model_name="default"):
        ...
        try:
            ...
            top5_results = [
                {
                    "class": (
                        idx_to_class.get(idx, "未知").split("_")[-1]
                        if "_" in idx_to_class.get(idx, "未知")
                        else idx_to_class.get(idx, "未知")
                    ),
            ...
            if (self.cache_hits + self.cache_misses) % 100 == 0:
                ...
                logger.info(
                    f"缓存统计: 命中 {self.cache_hits}, 未命中 {self.cache_misses}, 命中率 {hit_rate:.2f}"
                )
...
def create_app(model_manager):
    ...
    Returns:
        FastAPI: FastAPI应用实例
    """
    app = FastAPI(title="角色识别API", description="使用深度学习模型识别动漫角色", version="1.0.0")
    ...
    async def predict_batch(files: list[UploadFile] = File(...), model: str = Form("default")):
        ...
        try:
            ...
            for file in files:
                # 检查文件类型
                if not file.content_type or not file.content_type.startswith("image/"):
                    results.append({"file_name": file.filename, "error": "只支持图像文件"})
                    continue
                ...
                try:
                    result = model_manager.predict(content, model_name=model)
                    results.append({"file_name": file.filename, "result": result})
                except ValueError as e:
                    results.append({"file_name": file.filename, "error": str(e)})
                except Exception as e:
                    results.append({"file_name": file.filename, "error": f"预测失败: {str(e)}"})
