...
class CharacterDataset(Dataset):
    ...
    def __init__(self, root_dir, transform=None):
        ...
        logger.info(
            f"数据集初始化完成，包含 {len(self.class_to_idx)} 个类别，{len(self.images)} 张图像"
        )
...
class ArcFaceLoss(nn.Module):
    ...
    def forward(self, embedding, logits, labels):
        ...
        """
        # 计算余弦相似度
        cos_theta = nn.functional.cosine_similarity(
            embedding, nn.functional.normalize(logits, dim=1), dim=1
        )
        ...
        modified_logits = self.scale * (
            one_hot * target_logits.unsqueeze(1) + (1 - one_hot) * logits
        )
...
def train_model(args):
    ...
    model = MetricLearningModel(embedding_dim=args.embedding_dim, num_classes=num_classes).to(
        device
    )
