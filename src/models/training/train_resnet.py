...
class ResNetCharacterClassifier(nn.Module):
    ...
    def __init__(self, num_classes):
        ...
        self.backbone.fc = nn.Sequential(
            nn.Linear(512, 256), nn.ReLU(), nn.Dropout(p=0.3), nn.Linear(256, num_classes)
        )
...
def train_model(args):
    ...
    # 初始化模型
    model = ResNetCharacterClassifier(num_classes=len(train_dataset.class_to_idx)).to(device)
    ...
    for epoch in range(args.num_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            no_improve_epochs = 0
            model_path = os.path.join(args.output_dir, "character_classifier_resnet_best.pth")
            torch.save(
                {
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description="ResNet模型训练 - 针对小数据集")

    # 数据参数
    parser.add_argument(
        "--train_dir", type=str, default="data/augmented_dataset", help="训练集目录"
    )
    parser.add_argument("--val_dir", type=str, default="data/split_dataset/val", help="验证集目录")
    parser.add_argument("--output_dir", type=str, default="models_resnet", help="模型输出目录")
    ...
    logger.info(f"ResNet模型训练完成！最佳验证准确率: {best_acc:.4f}")
