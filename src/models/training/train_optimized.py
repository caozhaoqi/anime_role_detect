...
def train_single_fold(model, train_loader, val_loader, device, args, fold_idx):
    ...
    for epoch in range(args.num_epochs):
        ...
        logger.info(
            f"Fold {fold_idx+1}, Epoch {epoch+1}/{args.num_epochs}, "
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(
                {"model_state_dict": model.state_dict(), "val_acc": val_acc, "fold": fold_idx},
                model_path,
            )
        else:
            no_improve_epochs += 1
...
def train_with_cross_validation(args):
    ...
    train_transform = transforms.Compose(
        [
            ...
            transforms.RandomAffine(degrees=20, translate=(0.2, 0.2), scale=(0.7, 1.3), shear=15),
    ...
    for fold_idx, (train_indices, val_indices) in enumerate(kfold.split(full_dataset.images)):
        ...
        # 训练
        best_val_acc = train_single_fold(model, train_loader, val_loader, device, args, fold_idx)
...
def main():
    ...
    # 数据参数
    parser.add_argument("--data_dir", type=str, default="data/split_dataset/train", help="数据目录")
    parser.add_argument("--output_dir", type=str, default="models_optimized", help="模型输出目录")
