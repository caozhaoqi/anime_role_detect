...
def train_model(args):
    ...
    model = ImprovedCharacterClassifier(
        num_classes=len(train_dataset.class_to_idx), dropout_rate=args.dropout_rate
    ).to(device)
    ...
    with open(history_path, "w") as f:
        ...
        for epoch in range(len(train_losses)):
            f.write(
                f"{epoch+1},{train_losses[epoch]:.4f},{train_accs[epoch]:.4f},{val_losses[epoch]:.4f},{val_accs[epoch]:.4f}\n"
            )
