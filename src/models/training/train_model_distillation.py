    ...
    if model_type == "mobilenet":
        ...
        try:
            ...
            model = timm.create_model(
                "efficientnet_lite0", pretrained=True, num_classes=num_classes
            )
...
def train_distillation(args):
    ...
    for epoch in range(args.num_epochs):
        ...
        for images, labels in progress_bar:
            ...
            progress_bar.set_postfix(
                {"loss": loss.item(), "acc": (train_correct / train_total) * 100}
            )
        ...
        with torch.no_grad():
            ...
            for images, labels in progress_bar:
                ...
                progress_bar.set_postfix({"loss": hard_loss.item(), "acc": current_acc})
    ...
    logger.info(
        f"学生模型相对于教师模型的性能保持率: {best_val_accuracy / teacher_accuracy * 100:.2f}%"
    )
...
def main():
    ...
    parser.add_argument("--val-dir", type=str, required=True, help="验证数据目录")
    parser.add_argument("--output-dir", type=str, default="models/distillation", help="输出目录")

    # 模型参数
    parser.add_argument("--teacher-model-path", type=str, help="教师模型权重路径")
    parser.add_argument(
        "--student-model-type",
        type=str,
        default="mobilenet",
    ...
    parser.add_argument("--num-epochs", type=int, default=30, help="训练轮数")
    ...
    # 蒸馏参数
    parser.add_argument("--temperature", type=float, default=3.0, help="温度参数")
