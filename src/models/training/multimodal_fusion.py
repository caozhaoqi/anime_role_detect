...
class MultimodalDataset(Dataset):
    ...
    def __init__(
        self, data_dir, text_annotations=None, transform=None, tokenizer=None, max_length=128
    ):
...
def main():
    ...
    parser.add_argument(
        "--model-path", type=str, default="models/multimodal_model.pth", help="模型路径"
    )
    parser.add_argument(
        "--config-path", type=str, default="models/multimodal_config.json", help="配置路径"
    )
    parser.add_argument("--train-dir", type=str, default="data/train", help="训练数据目录")
    parser.add_argument(
        "--val-dir", type=str, default="data/split_dataset/val", help="验证数据目录"
    )
    parser.add_argument(
        "--output-model",
        type=str,
        default="models/multimodal_model_trained.pth",
        help="输出模型路径",
    )
    parser.add_argument(
        "--output-config",
        type=str,
        default="models/multimodal_config_trained.json",
        help="输出配置路径",
    )
    ...
    system = MultimodalFusionSystem(
        model_path=args.model_path, fusion_method=args.fusion_method, device=args.device
    )
    ...
    val_accuracy = system.evaluate(data_dir=args.val_dir, batch_size=args.batch_size)
