...
def train_model(
    model, train_loader, val_loader, criterion, optimizer, device, epochs, output_dir, dataset
):
    """训练模型"""
    best_val_acc = 0.0
...
def train_model_with_attributes(
    model,
    train_loader,
    val_loader,
    class_criterion,
    ...
    for epoch in range(epochs):
        ...
        for images, class_labels, attribute_labels in tqdm(
        ...
        logger.info(f"Epoch {epoch+1}/{epochs}")
        logger.info(
            f"Train Class Loss: {train_class_loss:.4f}, Train Attribute Loss: {train_attribute_loss:.4f}, Train Acc: {train_acc:.4f}"
        )
        logger.info(
            f"Val Class Loss: {val_class_loss:.4f}, Val Attribute Loss: {val_attribute_loss:.4f}, Val Acc: {val_acc:.4f}"
        )
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default="models/character_classifier", help="输出目录"
    )
    ...
    if args.annotations_file:
        ...
        dataset = CharacterAttributeDataset(
            args.data_dir, args.annotations_file, transform=transform
        )
