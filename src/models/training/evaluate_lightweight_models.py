...
def main():
    ...
    parser.add_argument("--data-dir", type=str, required=True, help="评估数据目录")
    ...
    for model_type in model_types:
        ...
        logger.info(
            f"准确率: {accuracy:.2f}%, 平均推理时间: {avg_inference_time:.2f} ms, FPS: {fps:.2f}"
        )
    ...
    for i, result in enumerate(results_sorted_by_accuracy[:3], 1):
        logger.info(
            f"{i}. {result['model_type']}: 准确率 {result['accuracy']:.2f}%, FPS {result['fps']:.2f}, 模型大小 {result['model_size_mb']:.2f} MB"
        )

    logger.info("\n按推理速度排序:")
    for i, result in enumerate(results_sorted_by_fps[:3], 1):
        logger.info(
            f"{i}. {result['model_type']}: FPS {result['fps']:.2f}, 准确率 {result['accuracy']:.2f}%, 模型大小 {result['model_size_mb']:.2f} MB"
        )

    logger.info("\n按模型大小排序:")
    for i, result in enumerate(results_sorted_by_size[:3], 1):
        logger.info(
            f"{i}. {result['model_type']}: 模型大小 {result['model_size_mb']:.2f} MB, 准确率 {result['accuracy']:.2f}%, FPS {result['fps']:.2f}"
        )
    ...
    logger.info(
        f"推理速度: {best_model['fps']:.2f} FPS ({best_model['avg_inference_time_ms']:.2f} ms/张)"
    )
