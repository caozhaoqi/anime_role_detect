...
class CharacterDataset(Dataset):
    ...
    def __init__(self, data_dir, transform=None):
        ...
        logger.info(
            f"数据集初始化完成，包含 {len(self.class_to_idx)} 个类别，{len(self.image_paths)} 张图像"
        )
...
class OnlineLearningSystem:
    ...
    def _load_model(self, model_path):
        ...
        model = CharacterClassifier(
            num_classes=self.num_classes, embedding_size=self.embedding_size
        )
    ...
    def _initialize_database(self):
        ...
        return {"index": index, "embeddings": [], "labels": [], "character_names": []}
    ...
    def _update_database(self, dataset):
        ...
        if self.character_database["embeddings"]:
            ...
            logger.info(
                f"特征数据库已更新，包含 {len(self.character_database['embeddings'])} 个特征"
            )
                ...
                for i in range(top_k):
                    if indices[0][i] < len(self.character_database["character_names"]):
                        ...
                        retrieval_results.append(
                            {"character": character_name, "distance": distances[0][i]}
                        )
            else:
                retrieval_results = []

        return {"classification": predicted_character, "retrieval": retrieval_results}
    ...
    def load_database(self, database_path):
        ...
        if os.path.exists(database_path):
            try:
                ...
                self.character_database["character_names"] = database_data.get(
                    "character_names", []
                )

                # 更新FAISS索引
                if self.character_database["embeddings"]:
                    embeddings_np = np.array(self.character_database["embeddings"]).astype(
                        "float32"
                    )
                    self.character_database["index"].reset()
                    self.character_database["index"].add(embeddings_np)
                ...
                logger.info(
                    f"特征数据库已加载，包含 {len(self.character_database['embeddings'])} 个特征"
                )
                return True
            except Exception as e:
...
def main():
    ...
    parser.add_argument(
        "--model-path", type=str, default="models/character_classifier.pth", help="模型路径"
    )
    parser.add_argument(
        "--database-path", type=str, default="models/character_database.json", help="特征数据库路径"
    )
    parser.add_argument(
        "--new-data-dir", type=str, default="data/new_characters", help="新数据目录"
    )
    parser.add_argument(
        "--output-model",
        type=str,
        default="models/character_classifier_updated.pth",
        help="更新后模型路径",
    )
    parser.add_argument(
        "--output-database",
        type=str,
        default="models/character_database_updated.json",
        help="更新后特征数据库路径",
    )
    ...
    parser.add_argument(
        "--device", type=str, default="cuda" if torch.cuda.is_available() else "cpu", help="设备"
    )

    args = parser.parse_args()

    # 初始化在线学习系统
    system = OnlineLearningSystem(model_path=args.model_path, device=args.device)
