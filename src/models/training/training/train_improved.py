...
def get_model(model_type, num_classes, dropout_rate=0.5):
    if model_type == "mobilenet_v2":
        ...
        model.classifier = nn.Sequential(
            nn.Dropout(p=dropout_rate), nn.Linear(model.classifier[1].in_features, num_classes)
        )
...
def main():
    ...
    parser.add_argument("--output-dir", type=str, default="../../models/improved", help="")
    ...
    full_dataset = CharacterDataset(
        args.data_dir, transform=train_transform, target_characters=target_characters
    )
