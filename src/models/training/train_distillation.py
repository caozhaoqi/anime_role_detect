...
class SimpleCharacterClassifier(nn.Module):
    ...
    def __init__(self, num_classes):
        ...
        self.backbone.classifier = nn.Sequential(
            nn.Linear(in_features, 256), nn.ReLU(), nn.Dropout(p=0.3), nn.Linear(256, num_classes)
        )
...
def train_with_distillation(args):
    ...
    teacher_model = ResNetCharacterClassifier(num_classes=len(train_dataset.class_to_idx)).to(
        device
    )
    ...
    # 初始化学生模型
    student_model = SimpleCharacterClassifier(num_classes=len(train_dataset.class_to_idx)).to(
        device
    )
