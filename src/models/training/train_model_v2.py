...
def train_model(args):
    ...
    train_dataset = datasets.ImageFolder(
        root="data/split_dataset_v2/train", transform=train_transform
    )

    val_dataset = datasets.ImageFolder(root="data/split_dataset_v2/val", transform=val_transform)
    ...
    for epoch in range(args.epochs):
        ...
        if val_acc > best_val_acc:
            ...
            logger.info(
                f"最佳模型已保存: models/character_classifier_best_v2.pth, 验证准确率: {best_val_acc:.4f}"
            )
        else:
