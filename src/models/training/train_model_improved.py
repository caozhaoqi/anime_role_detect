...
logger = logging.getLogger("train_model_improved")
...
class CharacterClassifier(nn.Module):
    ...
    def __init__(self, num_classes):
        ...
        """
        super().__init__()
        # 使用EfficientNet-B0作为基础模型，避免网络下载问题
...
def train_model(args):
    ...
    train_transform = transforms.Compose(
        [
            transforms.Resize((240, 240)),  # 稍大的输入尺寸
            ...
            transforms.ColorJitter(
                brightness=0.3,  # 更大的亮度变化
                contrast=0.3,  # 更大的对比度变化
                saturation=0.3,  # 更大的饱和度变化
                hue=0.1,  # 色调变化
            ),
    ...
    for epoch in range(args.num_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            model_path = os.path.join(args.output_dir, f"character_classifier_best_improved.pth")
            torch.save(
                {
    ...
    # 保存最终模型
    final_model_path = os.path.join(args.output_dir, "character_classifier_final_improved.pth")
    torch.save(
        {"model_state_dict": model.state_dict(), "class_to_idx": train_dataset.class_to_idx},
        final_model_path,
    )
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description="改进的角色分类模型训练")

    # 数据参数
    parser.add_argument(
        "--train_dir", type=str, default="data/split_dataset/train", help="训练集目录"
    )
