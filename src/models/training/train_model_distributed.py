...
def train_model(rank, world_size, args):
    ...
    for epoch in range(args.num_epochs):
        ...
        with tqdm(
            train_loader, desc=f"Epoch {epoch+1}/{args.num_epochs}", unit="batch", disable=rank != 0
        ) as pbar:
        ...
        if rank == 0:
            ...
            if avg_val_acc > best_val_acc:
                ...
                model_path = os.path.join(
                    args.output_dir, f"character_classifier_best_distributed.pth"
                )
                torch.save(
                    {
    ...
    if rank == 0:
        final_model_path = os.path.join(
            args.output_dir, "character_classifier_final_distributed.pth"
        )
        torch.save(
            {
                "model_state_dict": model.module.state_dict(),
...
def main():
    ...
    if world_size < 2:
        ...
        for epoch in range(args.num_epochs):
            ...
            with tqdm(
                train_loader, desc=f"Epoch {epoch+1}/{args.num_epochs}", unit="batch"
            ) as pbar:
                for inputs, labels in pbar:
                    inputs, labels = inputs.to(device), labels.to(device)
