...
def train_model(args):
    ...
    # 初始化模型
    model = EfficientNetB7CharacterClassifier(num_classes=len(train_dataset.class_to_idx)).to(
        device
    )
    ...
    for epoch in range(args.num_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            model_path = os.path.join(
                args.output_dir, "character_classifier_efficientnet_b7_best.pth"
            )
            torch.save(
                {
...
def main():
    ...
    parser.add_argument(
        "--output_dir", type=str, default="models_efficientnet_b7", help="模型输出目录"
    )
