...
def main():
    ...
    # 运行优化
    study.optimize(
        lambda trial: objective(trial, args), n_trials=args.n_trials, timeout=3600  # 1小时超时
    )
    ...
    with open(best_params_path, "w") as f:
        json.dump({"best_val_acc": best_trial.value, "params": best_trial.params}, f, indent=2)
    ...
    scheduler = optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=args.num_epochs * 2, eta_min=1e-6  # 增加训练轮数
    )
