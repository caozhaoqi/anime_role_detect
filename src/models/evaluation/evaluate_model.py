...
def test_single_image(model, image_path, transform, device, class_names, attribute_names=None):
    ...
    # 构建结果
    result = {"character": predicted_class, "confidence": class_confidence}
...
def main():
    ...
    if args.test_image:
        result = test_single_image(
            ...
            device,
            class_names,
            attribute_names if args.annotations_file else None,
        )
        logger.info("\n" + "=" * 50)
        logger.info("预测结果:")
        logger.info("=" * 50)
    ...
    # 评估模型
    if args.annotations_file:
        results = evaluate_model_with_attributes(
            model, test_loader, device, class_names, attribute_names
        )
    else:
