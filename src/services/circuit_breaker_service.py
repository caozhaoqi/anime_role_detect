...
class CircuitBreakerService:
    ...
    def execute_with_fallback(self, name: str, func: Callable, fallback: Callable, *args, **kwargs) -> Any:
        """执行函数，失败时使用降级策略"""
...
def execute_with_fallback(name: str, func: Callable, fallback: Callable, *args, **kwargs) -> Any:
    """执行函数，失败时使用降级策略"""
    return get_circuit_breaker_service().execute_with_fallback(name, func, fallback, *args, **kwargs)
