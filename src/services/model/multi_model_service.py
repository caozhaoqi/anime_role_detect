...
class MultiModelService:
    ...
    def __init__(self):
        ...
        # 模型配置
        self.model_configs = {
            "resnet18_loli8": {"type": "local", "weight": 0.3},
            "mobilenet_v2": {"type": "local", "weight": 0.25},
            "vit_b_16": {"type": "service", "weight": 0.45},
        }
    ...
    def _fuse_multi_role_results(
        ...
        for role_name, role_list in role_groups.items():
            ...
            weighted_similarity = (
                sum(
                    role.get("similarity", 0.0) * role.get("model_weight", 1.0)
            ...
            weighted_confidence = (
                sum(
                    role.get("confidence", 0.0) * role.get("model_weight", 1.0)
                    for role in role_list
                )
                / total_weight
            )
    ...
    def _weighted_average_fusion(
        ...
        weighted_similarity = (
            sum(result.get("similarity", 0.0) * weight for _, weight, result in results)
            / total_weight
        )
    ...
    def _majority_vote_fusion(
        ...
        similarities = [
            ...
            for _, _, result in results
            if result.get("role") == most_common_role
        ]
    ...
    def add_model(self, model_name: str, model_type: str, weight: float):
        ...
        Args:
            ...
            weight: 模型权重
        """
        self.model_configs[model_name] = {"type": model_type, "weight": weight}
...
def process_with_multiple_models(file, content, multi_role=False) -> Dict[str, Any]:
    ...
    """
    return asyncio.run(
        get_multi_model_service().process_with_multiple_models(file, content, multi_role)
    )
