...
class ModelVersionService:
    ...
    def __init__(self):
        ...
        # 模型版本配置文件
        self.config_file = os.path.join(os.path.dirname(__file__), '..', '..', 'config', 'model_versions.json')
    ...
    def register_model(self, model_name: str, version: str, path: str, description: str = "") -> bool:
        """注册模型版本"""
        try:
    ...
    def get_model_path(self, model_name: str, version: str = "latest") -> Optional[str]:
        ...
        if version == "latest":
            # 按创建时间排序，返回最新版本
            latest_version = sorted(versions.items(), key=lambda x: x[1].get("created_at", 0), reverse=True)[0][0]
            return versions[latest_version].get("path")
        
        return versions.get(version, {}).get("path")
    
    def enable_ab_test(self, test_models: List[Tuple[str, str]], weights: List[float], control_model: str):
        """启用A/B测试"""
        self.ab_test_config = {
            "enabled": True,
