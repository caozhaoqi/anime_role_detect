...
MODEL_INFERENCE_TIME = Histogram(
    'anime_role_detect_model_inference_seconds',
...
CACHE_HIT_RATE = Gauge(
    'anime_role_detect_cache_hit_rate',
    'Cache hit rate',
    ['cache_type']
)
...
# 错误计数
ERROR_COUNT = Counter(
    'anime_role_detect_errors_total',
...
SERVICE_HEALTH = Gauge(
    'anime_role_detect_service_health',
    'Service health status (1=healthy, 0=unhealthy)'
)

# 模型服务状态
MODEL_SERVICE_STATUS = Gauge(
    'anime_role_detect_model_service_status',
    'Model service status (1=healthy, 0=unhealthy)'
)
...
class MonitoringService:
    ...
    def record_multi_role_detection(self, success: bool):
        """记录多角色检测"""
        result = "success" if success else "failure"
        MULTI_ROLE_DETECTION_COUNT.labels(
            result=result
        ).inc()
