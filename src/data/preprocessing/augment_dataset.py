...
def augment_dataset(input_dir, output_dir, num_augmentations_per_image=10):
    ...
    for cls in tqdm(classes, desc="增强类别"):
        ...
        images = [
            f
            for f in os.listdir(cls_input_dir)
...
def main():
    ...
    parser.add_argument(
        "--input_dir", type=str, default="data/split_dataset/train", help="输入数据集目录"
    )
    parser.add_argument(
        "--output_dir", type=str, default="data/augmented_dataset", help="输出增强数据集目录"
    )
    parser.add_argument("--num_augmentations", type=int, default=20, help="每张图像的增强数量")
