        ...
        try:
            ...
            for aug_name, aug_img in augmentations[: self.augment_factor + 1]:
                aug_output_path = os.path.join(
                    os.path.dirname(output_path), f"{base_name_without_ext}_{aug_name}{ext}"
                )
    ...
    def augment_directory(self):
        ...
        for root, dirs, files in os.walk(self.input_dir):
            for dir_name in dirs:
                ...
                image_files = [
                    f for f in subdir_files if f.lower().endswith((".jpg", ".jpeg", ".png", ".bmp"))
                ]
...
def main():
    ...
    parser.add_argument(
        "--augment_factor", type=int, default=3, help="增强因子，每个图像生成的增强样本数"
    )
    ...
    augmentation = DataAugmentation(
        input_dir=args.input_dir, output_dir=args.output_dir, augment_factor=args.augment_factor
    )
