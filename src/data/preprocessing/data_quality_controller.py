...
class DataQualityController:
    def __init__(
        self, data_dir="data/train", min_resolution=512, min_aspect_ratio=0.7, max_aspect_ratio=1.4
    ):
        """
        初始化数据质量控制器
    ...
    def check_image_quality(self, image_path):
        ...
        try:
            ...
            if image.mode == "RGB":
                ...
                edge_density = (np.sum(gradient_x > 20) + np.sum(gradient_y > 20)) / (
                    gray_data.size
                )
    ...
    def validate_dataset_quality(self):
        ...
        character_dirs = [
            ...
            and d not in ["quality_reports", "low_quality"]
        ]
        ...
        for character_dir in tqdm(character_dirs):
            ...
            image_files = [
                f for f in os.listdir(character_path) if f.endswith((".jpg", ".jpeg", ".png"))
            ]
    ...
    def generate_quality_report(self, report):
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"- 通过质量检查: {report['passed_images']} ({report['passed_images']/report['total_images']*100:.2f}%)\n"
            )
            f.write(
                f"- 未通过质量检查: {report['failed_images']} ({report['failed_images']/report['total_images']*100:.2f}%)\n\n"
            )

            f.write(f"## 质量问题分布\n")
            for issue, count in sorted(
                report["quality_issues"].items(), key=lambda x: x[1], reverse=True
            ):
    ...
    def analyze_data_distribution(self):
        ...
        if distribution_report["total_characters"] > 0:
            distribution_report["average_images_per_character"] = (
                total_images / distribution_report["total_characters"]
            )
    ...
    def generate_distribution_report(self, distribution_report):
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"- 平均每个角色图像数: {distribution_report['average_images_per_character']:.2f}\n\n"
            )

            f.write(f"## 角色图像分布\n")
            sorted_characters = sorted(
                distribution_report["character_distribution"].items(),
                key=lambda x: x[1],
                reverse=True,
            )
            ...
            sorted_resolutions = sorted(
                distribution_report["resolution_distribution"].items(),
                key=lambda x: x[1],
                reverse=True,
            )[:20]
            ...
            for resolution, count in sorted_resolutions:
                f.write(
                    f"- {resolution}: {count} 张 ({count/distribution_report['total_images']*100:.2f}%)\n"
                )
            f.write("\n")
    ...
    def cleanup_low_quality(self):
        ...
        logger.info(
            f"质量检查结果: {quality_report['passed_images']} 张通过, {quality_report['failed_images']} 张未通过"
        )
        logger.info(f"移动低质量图像: {moved_count} 张")
        logger.info(
            f"数据集分布: 平均每个角色 {distribution_report['average_images_per_character']:.2f} 张图像"
        )
...
def main():
    ...
    controller = DataQualityController(data_dir=args.data_dir, min_resolution=args.min_resolution)
