...
def split_dataset(input_dir, output_dir, train_ratio=0.8, min_samples=10):
    ...
    for character in characters:
        ...
        if len(image_files) < min_samples:
            ...
            logger.warning(
                f"角色 {character} 样本数量不足 ({len(image_files)} < {min_samples})，已过滤"
            )
            continue
        ...
        logger.info(
            f"角色 {character}: 总图像 {len(image_files)}, 训练集 {len(train_files)}, 验证集 {len(val_files)}"
        )

    logger.info(f"数据集分割完成！")
    logger.info(
        f"总角色数: {len(characters)}, 过滤角色数: {filtered_characters}, 有效角色数: {len(characters) - filtered_characters}"
    )
