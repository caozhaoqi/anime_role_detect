    ...
    def collect_supplement_data(self, threshold=50, max_images_per_character=100):
        ...
        for char_info in insufficient_characters:
            ...
            try:
                ...
                # 实际项目中可能需要更精确的控制
                result = self.collector.collect_from_keywords(
                    series_name, max_images=max_images_per_character
                )
...
def main():
    ...
    parser.add_argument("--threshold", type=int, default=50, help="数据不足的阈值")
    parser.add_argument("--max-images", type=int, default=100, help="每个角色的最大图像数")
    parser.add_argument(
        "--auto-spider-img-dir", type=str, default="auto_spider_img", help="关键词文件目录"
    )

    args = parser.parse_args()

    # 初始化采集器
    collector = DataSupplementCollector(auto_spider_img_dir=args.auto_spider_img_dir)

    # 开始采集
    collector.collect_supplement_data(
        threshold=args.threshold, max_images_per_character=args.max_images
    )
