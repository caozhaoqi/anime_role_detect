...
class FastImageCollector:
                ...
                try:
                    ...
                    for img in soup.find_all("img"):
                        ...
                        if (
                            ...
                            and not img_url.startswith("data:")
                        ):
                            # 确保URL是完整的
    ...
    def collect_images(self, characters):
        ...
        for character in tqdm(characters, desc="采集角色图像"):
            ...
            # 计算现有图像数量
            existing_images = len(
                [f for f in os.listdir(char_dir) if f.lower().endswith((".jpg", ".jpeg", ".png"))]
            )
            ...
            for query in search_queries:
                ...
                # 批量下载
                batch_collected = self.download_image_batch(
                    image_urls, char_dir, character, existing_images + collected
                )
                collected += batch_collected

                logger.info(
                    f"搜索关键词: {query}, 成功下载 {batch_collected} 张图像，总计 {collected}/{need_images}"
                )
            ...
            logger.info(
                f"角色 {character} 采集完成，新增 {collected} 张图像，总计 {existing_images + collected} 张"
            )
...
def main():
    ...
    collector = FastImageCollector(
        output_dir=output_dir, max_images_per_character=200, max_workers=8  # 并发下载线程数
    )
