...
class CharacterNameCollector:
    ...
    def load_characters_from_directory(self, directory_path: str) -> Dict[str, List[str]]:
        ...
        try:
            ...
            logger.info(
                f"从 {directory_path} 加载了 {total} 个角色名称，共 {len(characters_dict)} 个文件"
            )
        except Exception as e:
    ...
    def filter_characters(
        self, characters: List[str], min_length: int = 2, max_length: int = 30
    ) -> List[str]:
        """
        过滤角色名称
    ...
    def get_statistics(self, characters_dict: Dict[str, List[str]]) -> Dict:
        ...
        file_stats = {name: len(characters) for name, characters in characters_dict.items()}
