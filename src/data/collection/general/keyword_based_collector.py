...
class KeywordBasedDataCollector:
    ...
    def _download_image(self, url, save_path, character_name=None):
        ...
        try:
            ...
            if content_analysis["content_type"] in ["small_image", "extreme_aspect_ratio"]:
                logger.warning(
                    f"图片内容类型不符合要求: {content_analysis['content_type']}, 跳过保存"
                )
                return False

            # 计算内容相关性（如果提供了角色名称）
            if character_name:
                relevance_score = self.image_utils.calculate_content_relevance(
    ...
    def _process_character(self, series, character_name, max_images=100):
        ...
        for i, img in enumerate(all_images[:needed_count]):
            save_path = os.path.join(
                character_dir, f"{series}_{character_name}_{existing_count + i:04d}.jpg"
            )
            future = self.concurrency_manager.submit(
                self._download_image, img["url"], save_path, character_name
            )
    ...
    def get_dataset_statistics(self):
        ...
        Returns:
            统计信息字典
        """
        statistics = {"total_characters": 0, "total_images": 0, "characters": {}}
    ...
    def validate_dataset(self):
        ...
        logger.info(
            f"数据集验证完成，检查了 {total_count} 张图像，删除了 {invalid_count} 张无效图像"
        )
...
def main():
    ...
    parser.add_argument("--output-dir", type=str, default="data/train", help="输出目录")
    parser.add_argument("--max-workers", type=int, default=5, help="最大并发数")
    parser.add_argument("--max-images", type=int, default=100, help="每个角色的最大图像数")
    parser.add_argument(
        "--series",
        type=str,
        help="指定要采集的系列 (genshin_chinese, genshin_english, star_rail_chinese, star_rail_english, honkai3_english, honkai3_chinese)",
    )
    parser.add_argument("--all-series", action="store_true", help="采集所有系列的数据")
    parser.add_argument("--validate", action="store_true", help="验证数据集")
    parser.add_argument("--stats", action="store_true", help="获取数据集统计信息")

    args = parser.parse_args()

    # 初始化采集器
    collector = KeywordBasedDataCollector(output_dir=args.output_dir, max_workers=args.max_workers)
    ...
    if args.stats:
        ...
        for character, count in sorted(
            stats["characters"].items(), key=lambda x: x[1], reverse=True
        )[:20]:
