...
class WDViTV3Tagger:
    ...
    def get_tags_local(self, image_path):
        ...
        try:
            ...
            # 预处理图片和文本
            inputs = self.processor(text=self.tags, images=image, return_tensors="pt", padding=True)
...
def process_directory(directory, output_dir, tagger):
    ...
    for image_path in tqdm(image_files, desc="处理图片"):
        try:
            ...
            # 保存标签为JSON
            tag_data = {"image_path": relative_path, "tags": tags, "tag_count": len(tags)}
...
def main():
    ...
    parser.add_argument(
        "--input-dir", type=str, default="../data/downloaded_images", help="输入图片目录"
    )
