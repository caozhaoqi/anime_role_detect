...
def create_annotations(data_dir, output_file, config_path=None):
    ...
    for character in os.listdir(data_dir):
        ...
        # 将英文角色名称映射到中文名称
        character_cn = character_mapping.get(character, character)

        # 检查角色是否在属性定义中
        if character_cn not in characters:
            skipped_characters.append(character)
            logger.warning(f"角色 {character} (中文: {character_cn}) 不在属性定义中，跳过")
            continue
...
def main():
    ...
    parser.add_argument(
        "--data-dir", type=str, default="../data/downloaded_images", help="数据目录"
    )
    parser.add_argument(
        "--output-file",
        type=str,
