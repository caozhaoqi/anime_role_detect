...
class KeypointAnnotator:
    ...
    def process_images(self):
        ...
        for root, dirs, files in os.walk(self.data_dir):
            ...
            # 处理当前目录下的图像
            images = [
                f for f in files if f.lower().endswith((".jpg", ".jpeg", ".png", ".bmp", ".webp"))
            ]
            ...
            if images:
                ...
                annotation_file = os.path.join(
                    self.output_dir, f"{rel_path.replace('/', '_')}_keypoints.json"
                )
...
def main():
    ...
    parser.add_argument(
        "--data-dir", type=str, default="data/downloaded_images", help="训练数据目录"
    )
    parser.add_argument(
        "--output-dir", type=str, default="data/keypoint_annotations", help="标注输出目录"
    )
    ...
    annotator = KeypointAnnotator(
        data_dir=args.data_dir, output_dir=args.output_dir, visualize=args.visualize
    )
