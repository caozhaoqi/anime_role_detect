...
INFERENCE_TIME = Histogram(
    "anime_role_detect_inference_duration_seconds", "Time spent on model inference", ["model_name"]
)

# API 响应时间直方图
API_RESPONSE_TIME = Histogram(
    "anime_role_detect_api_response_duration_seconds", "API response time", ["endpoint"]
)

# 内存使用量
MEMORY_USAGE = Gauge("anime_role_detect_memory_usage_bytes", "Current memory usage", ["type"])

# GPU 内存使用量
GPU_MEMORY_USAGE = Gauge("anime_role_detect_gpu_memory_usage_bytes", "Current GPU memory usage")

# 模型加载状态
MODEL_LOADED = Gauge(
    "anime_role_detect_model_loaded", "Whether the model is loaded", ["model_name"]
)

# 请求成功率
SUCCESS_RATE = Summary("anime_role_detect_success_rate", "Request success rate")
