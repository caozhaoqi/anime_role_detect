...
def convert_tags_to_annotations(tags_file, output_file):
    ...
    for item in tags_data:
        ...
        # 添加到标注列表
        annotations.append(
            {"character": character, "image_path": image_path, "attribute_labels": attribute_labels}
        )
...
def main():
    ...
    parser.add_argument(
        "--tags-file", type=str, default="data/tags.json", help="生成的标签文件路径"
    )
    parser.add_argument(
        "--output-file", type=str, default="data/annotations.json", help="输出的标注文件路径"
    )
