...
def main():
    ...
    parser.add_argument(
        "--annotations-file", type=str, default="data/annotations.json", help="标注文件路径"
    )
    parser.add_argument(
        "--attributes-file",
        type=str,
        default="config/character_attributes.json",
        help="属性配置文件路径",
    )
    parser.add_argument("--tags-file", type=str, default=None, help="标签文件路径")
    parser.add_argument(
        "--output-file", type=str, default="data/processed_annotations.json", help="输出文件路径"
    )
