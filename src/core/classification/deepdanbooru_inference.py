...
class DeepDanbooruInference:
    ...
    def predict(self, image_path, top_k=10, threshold=0.5):
        ...
        try:
            ...
            with open(image_path, "rb") as f:
                ...
                data = {"top_k": top_k, "threshold": threshold}
                ...
                if "data" in result and isinstance(result["data"], list):
                    for item in result["data"]:
                        if isinstance(item, dict) and "tag" in item and "score" in item:
                            tags.append({"tag": item["tag"], "score": item["score"]})
    ...
    def predict_batch(self, image_paths, batch_size=4, top_k=10, threshold=0.5):
        ...
        try:
            ...
            for i in range(0, len(image_paths), batch_size):
                ...
                for img_path in batch_paths:
                    try:
                        ...
                        results.append({"image_path": img_path, "tags": tags})
                    except Exception as e:
            ...
            print(
                f"DeepDanbooru批量处理完成: {len(image_paths)}张图像，平均每张 {avg_time_per_image:.4f}秒"
            )
    ...
    def predict_parallel(self, image_paths, num_workers=4, top_k=10, threshold=0.5):
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=num_workers) as executor:
                # 提交所有预测任务
                future_to_path = {
                    executor.submit(self.predict, path, top_k, threshold): path
            ...
            avg_time_per_image = total_time / len(image_paths)
            print(
                f"DeepDanbooru并行处理完成: {len(image_paths)}张图像，平均每张 {avg_time_per_image:.4f}秒"
            )
